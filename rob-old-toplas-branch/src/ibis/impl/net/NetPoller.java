...
public class NetPoller extends NetInput implements NetBufferedInputSupport {
    ...
    private void setSingleton(ReceiveQueue q, boolean on) throws IOException {
        ...
        if (VERBOSE_SINGLETON) {
            System.err.println(this + ": SINGLETON_FASTPATH="
                    + SINGLETON_FASTPATH + " on=" + on + " upcallFunc="
                    + upcallFunc + " now singleton=" + singleton + " current="
                    + q.getInput());
        ...
        if (SINGLETON_FASTPATH && on) {
            ...
            if (singleton != null) {
                if (VERBOSE_SINGLETON) {
                    System.err.println(this
                            + ": Retain singleton fastpath, subInput "
                            + q.getInput());
                }
                return;
            }
            ...
            try {
                ...
                if (q.pollIsInterruptible()) {
                    ...
                    if (VERBOSE_SINGLETON) {
                        System.err.println(this
                                + ": Set singleton fastpath, subInput "
                                + q.getInput());
                    }
                    singleton = q;
            ...
            if (VERBOSE_SINGLETON) {
                System.err.println(this + ": Disable singleton "
                        + singleton.input + " fastpath; add " + ni);
            }

            if (q == singleton) {
                throw new Error(this
                        + ": cannot be multipoller and also be the singleton "
                        + singleton);
            }
     ...
     */
    protected synchronized void setupConnection(NetConnection cnx, Object key)
            throws IOException {
        ...
        if (createNewSubInput) {
            ...
            ni = q.getInput();
            if (VERBOSE_SINGLETON) {
                System.err.println(this + ": recycle existing " + q
                        + " for key " + key);
            }
        }
    ...
    public void interruptPoll() throws IOException {
        if (singleton != null) {
            if (singleton.pollIsInterruptible()) {
                ...
                throw new Error(
                        "interruptPoll on singleton that is nonInterruptible");
            }
        } else {
    ...
    synchronized public void switchToUpcallMode(NetInputUpcall inputUpcall)
        ...
        if (singleton != null) {
            if (singleton.pollIsInterruptible()) {
                ...
                throw new Error(
                        "switchToUpcallMode invoked on nonInterruptible singleton");
            }
        } else {
            ...
            while (i.hasNext()) {
                ...
                if (ni.getUpcallFunc() == null) {
                    throw new Error(this + ": switch nonSingleton subInput "
                            + rq.getInput() + " to upcallMode");
                }
    ...
    protected void verifyNonSingletonPoller() {
        if (singleton != null) {
            throw new Error(
                    "Cannot be a singleton Poller AND have upcall threads");
        }
    }
  ...
  //              }
        ...
        synchronized void interruptDeliveringThread() {
            if (waitingPollers > 0) {
                notifyAll();
            }
        }
    ...
    protected void selectConnection(ReceiveQueue rq) {
        ...
        if (ibis.impl.net.NetIbis.DEBUG_RUTGER) {
            System.err.println(this + ": selectConnection, input " + input
        ...
        while (singleton == null) {
            ...
            if (s != 0) {
                ...
                if (spn == null) {
                    ...
                    i = c.iterator();
                    j = 0;
                    while (spn == null && j++ < firstToPoll && i.hasNext()) {
                        rq = (ReceiveQueue) i.next();
                        spn = rq.poll();
                    }
                }
                ...
                if (spn != null) {
                    ...
                    if (ibis.impl.net.NetIbis.DEBUG_RUTGER) {
                        System.err.println(this
                                + ": pollNonSingleton sets activeQueue "
                                + activeQueue);
                    }
                    selectConnection(rq);
                    break;
            ...
            synchronized (this) {
                if (activeQueue != null) {
                    throw new IOException(
                            "Call message.finish before calling Net.poll");
                }
