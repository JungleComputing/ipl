...
provides a method \texttt{elect} which (globally) selects one of
...
The Ibis nameserver is started with the ibis\_nameserver script which
lives in the Ibis bin directory. Before starting an Ibis application,
...
option to the ibis\_nameserver script. This script recognizes
...
An Ibis instance is started with the run\_ibis script which lives in
the Ibis bin directory.
This run\_ibis script is called as follows:
\begin{center}
run\_ibis \emph{procno nprocs id port ns javaflags class ...}
\end{center}
...
The run\_ibis script uses these parameters to set the following
...
of a Satin job: they must be of a basic type or serializable.
