...
When a receive port is created, it will not accept connections until
the \verb+enableConnections+ method is invoked. So, the creator of
the receive port can determine when he is ready to accept connections.
Also, if the receive port is configured for upcalls, these must
explicitly be enabled by calling \verb+enableUpcalls+.
