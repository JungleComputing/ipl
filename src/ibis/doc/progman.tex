...
The IPL consists of a set of Java interfaces that define how an Ibis program
can make use of the Ibis components.
The Ibis program does not need to know which specific Ibis implementations are
available.
...
receive ports.
To send a message, the Ibis program has to obtain a
...
\item[upcalls]
when a receive port is configured for upcalls, the Ibis program provides
...
\item[Reliable]
reliable communication is supported, that is, messages don't get lost.
A reliable communication protocol is used.
\item[Unreliable]
unreliable communication is explicitly supported, that is, an
unreliable communication protocol is used, and messages may get lost.
TODO: Should we keep this property or leave it out in which case
not having "Reliable" means: don't care?
\item[AutoUpcalls]
