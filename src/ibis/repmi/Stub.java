...
public class Stub implements java.io.Serializable, ibis.io.Serializable {
    ...
    public void generated_WriteObject(
            IbisSerializationOutputStream mantaoutputstream) throws IOException {
        mantaoutputstream.writeInt(objectID);
    }

    public void generated_DefaultWriteObject(
            IbisSerializationOutputStream mantaoutputstream, int dummy)
            throws IOException {
        mantaoutputstream.writeInt(objectID);
    }

    public void generated_ReadObject(
            IbisSerializationInputStream mantainputstream) throws IOException {
        objectID = mantainputstream.readInt();
        localSkeleton = RTS.findSkeleton(objectID);
    }

    public void generated_DefaultReadObject(
            IbisSerializationInputStream mantainputstream, int dummy)
            throws IOException {
        objectID = mantainputstream.readInt();
        localSkeleton = RTS.findSkeleton(objectID);
    }
