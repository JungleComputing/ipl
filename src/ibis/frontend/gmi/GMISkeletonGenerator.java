...
class GMISkeletonGenerator extends GMIGenerator {
    ...
    void header() {
        ...
        output.println("public final class group_skeleton_" + dest_name
                + " extends ibis.gmi.GroupSkeleton {");
        output.println();
        output.println("public String toString() { return \"group_skeleton_"
                + dest_name + "\";}");
        output.println();
    }
    ...
    void handleResult(String spacing, Type ret) {
        ...
        output.println(spacing
                + "\t\tw = Group.unicast(cpu_rank).newMessage();");
        ...
        if (!ret.equals(Type.VOID)) {
            ...
            if (ret instanceof ArrayType) {
                output.println(spacing + "\t\t\t" + getType(ret)
                        + "[] out = new " + getArrayType((ArrayType) ret, "1")
                        + "[];");
            } else {
                output.println(spacing + "\t\t\t" + getType(ret)
                        + "[] out = new " + getType(ret) + "[1];");
            }
            output.println(spacing
                    + "\t\t\t\tpersonalizer.personalize(result, out);");
        ...
        output.println(spacing + "\t\tif (personalizer != null) {");
        output.println(spacing
                + "\t\t\texcpts = new Exception[info.numInvokers];");
        if (!ret.equals(Type.VOID)) {
            if (ret instanceof ArrayType) {
                output.println(spacing + "\t\t\tout = new "
                        + getArrayType((ArrayType) ret, "info.numInvokers")
                        + "[];");
            } else {
                output.println(spacing + "\t\t\tout = new " + getType(ret)
                        + "[info.numInvokers];");
            }
        }
        output.println(spacing + "\t\t\tif (ex != null) {");
        output.println(spacing
                + "\t\t\t\tpersonalizer.personalize(ex, excpts);");
        if (!ret.equals(Type.VOID)) {
            output.println(spacing + "\t\t\t} else {");
            output.println(spacing
                    + "\t\t\t\tpersonalizer.personalize(result, out);");
    ...
    void handleMethodInvocation(String spacing, Method m, Type ret,
        ...
        for (int j = 0; j < params.length; j++) {
            ...
            if (temp instanceof BasicType) {
                output.println(readMessageType(spacing + "\t", "p" + j, "r",
                        temp, true));
            } else {
    ...
    void methodHandler(String spacing, Method m) {
        ...
        output.println(spacing
                + "\tif (resultMode >= ReplyScheme.R_PERSONALIZED) {");
        output.println(spacing
                + "\t\tresultMode -= ReplyScheme.R_PERSONALIZED;");
        output.println(spacing + "\t\ttry {");
        output.println(spacing
                + "\t\t\tpersonalizer = (ReplyPersonalizer) r.readObject();");
        output.println(spacing + "\t\t} catch(ClassNotFoundException e) {");
        output.println(spacing
                + "\t\t\tthrow new Error(\"Expected ReplyPersonalizer\", e);");
        output.println(spacing + "\t\t}");
        output.println(spacing + "\t}");
        ...
        output.println(spacing + "\t\ttry {");
        output.println(spacing
                + "\t\t\tcombiner = (BinomialCombiner) r.readObject();");
        output.println(spacing + "\t\t} catch(ClassNotFoundException e) {");
        output.println(spacing
                + "\t\t\tthrow new Error(\"Expected BinomialCombiner\", e);");
        output.println(spacing + "\t\t}");
        ...
        if (ret.equals(Type.VOID)) {
            output.println(spacing
                    + "\t\t\tcombine_void(combiner, false, cpu_rank, ex);");
        } else {
