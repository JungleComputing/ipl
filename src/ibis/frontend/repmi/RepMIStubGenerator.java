...
class RepMIStubGenerator extends RepMIGenerator {
    ...
    void methodBody(Method m, int number) {
        ...
        if (!write) {
            ...
            output.print("((" + data.classname + ")localSkeleton.destination)."
                    + m.getName() + "(");
            for (int j = 0; j < params.length; j++) {
            ...
            for (int j = 0; j < params.length; j++) {
                output.println(writeMessageType("\t\t\t", "w", params[j], "p"
                        + j));
            }
    ...
    void header() {
        ...
        output.print("public final class repmi_stub_" + data.classname
                + " extends ibis.repmi.Stub implements ");
