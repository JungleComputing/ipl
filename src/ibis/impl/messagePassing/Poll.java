...
final public class Poll implements Runnable {
    ...
    static {
        ...
        USE_SLEEP_FOR_YIELD = !TypedProperties.booleanProperty(MPProps.s_yield)
                && !TypedProperties.stringPropertyMatch("java.compiler",
                        "manta");
    ...
    void report(java.io.PrintStream out) {
        if (STATISTICS) {
            out.println(Ibis.myIbis.myCpu + ": Poll: /preempt " + poll_preempt
                    + " /non-preempt " + poll_non_preempt + " poll "
                    + poll_poll + " (direct " + poll_poll_direct + ") wait "
                    + poll_wait + " yield/preempt " + poll_yield_preempt
