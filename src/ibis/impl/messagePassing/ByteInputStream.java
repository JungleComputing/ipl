...
final class ByteInputStream extends ibis.io.ArrayInputStream implements
    ...
    void setMsgHandle(ReadMessage msg) {
        ...
        if (Ibis.DEBUG) {
            System.err.println(Thread.currentThread()
                    + "ByteInputStream.msgHandle := "
                    + Integer.toHexString(msgHandle) + " msgSize := " + msgSize
                    + " msg " + msg);
        }
    }
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        try {
            if (Ibis.DEBUG) {
                System.err.println("Now want to read " + len
                        + " bytes, avaible in fragment "
                        + Integer.toHexString(msgHandle) + ": " + msgSize);
                Thread.dumpStack();
            }
            if (msgSize == 0) {
                msg.nextFragment();
                if (Ibis.DEBUG) {
                    System.err.println("After nextFragment() want to read "
                            + len + " bytes, avaible in fragment "
                            + Integer.toHexString(msgHandle) + ": " + msgSize);
                }
            }
    ...
    public void readArray(boolean b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(boolean[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(char b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(char[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(short b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(short[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(int b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(int[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(long b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(long[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(float b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(float[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
    ...
    public void readArray(double b[], int off, int len) throws IOException {
        ...
        try {
            while (len > 0) {
                ...
                if (Ibis.DEBUG && msgSize < 0) {
                    throw new ArrayIndexOutOfBoundsException(
                            "readArray(double[]): insufficient data");
                }
                len -= rd;
                off += rd;
            }
