...
public class NetServiceInputStream extends InputStream {
	...
	private static class PooledThread extends Thread {
	    ...
	    static PooledThread[]	pool = new PooledThread[poolSize];

	    static {
		for (int i = 0; i < poolSize; i++) {
		    pool[i] = new PooledThread();
		    pool[i].start();
		}
	    }
		...
		synchronized (pool) {
		    if (poolPtr == poolSize) {
			// System.err.println("Exceed NetServiceIS pool size");
		...
		while (true) {
			...
			if (poolPtr > 0) {
			    ...
			    /* No afterlife if the pool is full */
			    return;
			}
		    }
		}
	    ...
	    void spawn(Runnable r, String name) {
		runnable = r;
		setName(name);
		synchronized (this) {
		    wokenUp = true;
	...
	private void spawnPopup() {
		...
		public void run() {
			...
			try {
			    ...
			    System.err.println(NetServiceInputStream.this + ": Popup thread gets " + e);
			}
			if (DEBUG) {
