        ...
        try {
            ...
            if (DEBUG) {
                System.err.println(NetIbis.hostName()
                        + " Thread.interrupt() -> interrupted Id " + id
                        + "; intpts " + lock[id].interrupts);
            }
            throw new InterruptedIOException(e);
        }
        if (lock[id].interrupts > interrupts) {
            if (DEBUG) {
                System.err.println(NetIbis.hostName() + " interrupted Id " + id
                        + "; intpts " + lock[id].interrupts);
            }
    ...
    private void interruptLocked(int id) {
        ...
        while (w != null) {
            ...
            if (w.id != 0 && lck.interrupts < interrupts) {
                if (DEBUG) {
                    System.err.println(NetIbis.hostName()
                            + ": interrupt dependent id " + w.id + "; intpts "
                            + interrupts);
                }
        ...
        if (lck.front != null && lck.interrupts < interrupts && id != 0) {
            if (DEBUG) {
                System.err.println(NetIbis.hostName() + ": interrupt own id "
                        + id + "; intpts " + interrupts);
            }
            lck.interrupts = interrupts;
            lck.cv.cv_bcast();
        }
