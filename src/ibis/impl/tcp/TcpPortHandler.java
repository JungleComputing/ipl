...
final class TcpPortHandler implements Runnable, TcpProtocol, Config {
    ...
    TcpPortHandler(TcpIbis me, VirtualSocketFactory fac)
        ...
        /*
         * We don't pass properties, since this is not a socket that is used for
         * an ibis port
         */
    ...
    ReceivePortIdentifier connect(TcpSendPort sp,
        ...
        try {
            ...
            // just rethrow the original exception, otherwise, we loose the
            // type! --Rob
            throw e; // new ConnectionRefusedException("Could not connect: "
            // + e);
        }
    }
    ...
    void quit() {
        try {
            quiting = true;
            /*
             * Connect to the serversocket, so that the port handler thread
             * wakes up.
             */
    ...
    private synchronized TcpReceivePort findReceivePort(String name) {
    ...
    private void handleRequest(VirtualSocket s) throws Exception {
        ...
        DataOutputStream data_out = new DataOutputStream(
                new BufferedOutputStream(new DummyOutputStream(out)));
        ...
        TcpSendPortIdentifier send = (TcpSendPortIdentifier) Conversion
            .byte2object(sp);
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                /*
                 * if the accept itself fails, we have a fatal problem. Close
                 * this receiveport.
                 */
                try {
                    System.err
