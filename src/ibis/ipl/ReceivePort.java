 ...
 * {@link ReceivePortConnectUpcall#gotConnection(ReceivePort, SendPortIdentifier)} upcall
 ...
 * {@link ReceivePortConnectUpcall#lostConnection(ReceivePort, SendPortIdentifier, Exception)}
 * upcall is performed. Both upcalls are completely
 * asynchronous, but Ibis ensures that at most one is active at any given
 * time.
 *
 * If no {@link ReceivePortConnectUpcall} is registered, the user is NOT informed
 ...
 * responsibility to use the {@link #lostConnections()}
...
public interface ReceivePort { 
     ...
     * A receiveport maintains a counter of how many bytes are used for messages.
    ...
    /**
     * Returns the {@link ibis.ipl.DynamicProperties DynamicProperties} of this ibis implementation.
     ...
     * This call is meaningless (and a no-op) for receiveports that were created
    ...
    /**
     * Prohibits upcalls.
     * After this call, no upcalls will occur until {@link #enableUpcalls()} is called.
     ...
     * Important: this method blocks until all sendports that are connected to it have been
     * freed. 
     */
    ...
    /** 
     * Frees the resources held by the receiveport. 
     * Important: this call does not block until all sendports that are connected to it
     * have been freed. Therefore, messages may be lost! Use this with extreme caution!
     * When this call is used, and this port is configured to maintain connection
     * administration, it updates the administration and thus may generate
     ...
     * When the free does not succeed within the timeout, this operation does a
     * {@link #forcedClose()}.
     * When this call is used, and this port is configured to maintain connection
     * administration, it updates the administration and thus may generate
     * lostConnection upcalls.
     ...
     * Connections can be lost due to an error, or because the sender disconnected. 
     ...
     * This call only works if this port is configured to maintain connection administration.
