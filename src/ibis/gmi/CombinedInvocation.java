...
public class CombinedInvocation extends InvocationScheme {
    ...
    public CombinedInvocation(String identifier, int rank, int size,
        ...
        if (id == null) {
            throw new ConfigurationException("Invalid operation identifier "
                    + id);
        }
        if (size < 1) {
            throw new ConfigurationException("Invalid number of participants "
                    + size + " (must be >= 1)");
        }
     ...
     */
    public CombinedInvocation(String identifier, int rank, int size,
            BinomialInvocationCombiner combiner, InvocationScheme inv)
            throws ConfigurationException {
