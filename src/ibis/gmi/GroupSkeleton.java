...
public abstract class GroupSkeleton implements GroupProtocol {
    ...
    static private final class GroupMessageQueue {
        ...
        public GroupMessage enqueue(int from) {
            logger.debug("Got message from cpu " + from);
        ...
        public GroupMessage dequeue(int from) {
            ...
            logger.debug("Waiting for message from cpu " + from);
     ...
     */
    public synchronized final void handleCombineMessage(ReadMessage m)
    ...
    protected final synchronized float combine_float(BinomialCombiner combiner,
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                logger.debug("Sending message to peer " + peer + " on cpu "
                        + peer_rank);
     ...
     */
    protected final synchronized double combine_double(
            BinomialCombiner combiner, boolean to_all, int lroot,
            double local_result, Exception ex) throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.doubleResult,
                                message.exceptionResult, groupSize);
     ...
     */
    protected final synchronized long combine_long(BinomialCombiner combiner,
            boolean to_all, int lroot, long local_result, Exception ex)
            throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.longResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized int combine_int(BinomialCombiner combiner,
            boolean to_all, int lroot, int local_result, Exception ex)
            throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.intResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized short combine_short(BinomialCombiner combiner,
            boolean to_all, int lroot, short local_result, Exception ex)
            throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.shortResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized char combine_char(BinomialCombiner combiner,
            boolean to_all, int lroot, char local_result, Exception ex)
            throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.charResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized byte combine_byte(BinomialCombiner combiner,
            boolean to_all, int lroot, byte local_result, Exception ex)
            throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.byteResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized boolean combine_boolean(
            BinomialCombiner combiner, boolean to_all, int lroot,
            boolean local_result, Exception ex) throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.booleanResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
     ...
     */
    protected final synchronized Object combine_Object(
            BinomialCombiner combiner, boolean to_all, int lroot,
            Object local_result, Exception ex) throws Exception {
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    try {
                        local_result = combiner.combine(myGroupRank,
                                local_result, ex, peer, message.objectResult,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
        ...
        while (mask < groupSize) {
            if ((mask & relrank) == 0) {
                ...
                if (peer < groupSize) {
                    ...
                    /* call the combiner */
                    try {
                        combiner.combine(myGroupRank, ex, peer,
                                message.exceptionResult, groupSize);
                        ex = null;
                    } catch (Exception e) {
                        ex = e;
