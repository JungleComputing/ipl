...
public final class Group implements GroupProtocol { 
	...
	GroupStub newStub() { 
		...
		e.printStackTrace();
		throw new RuntimeException(Group._rank + " GroupStubData.newStub() Failed to create stub of type " + typeName + " for group " + groupName + " " + e);			
	    } 
	} 
    } 
     ...
     */
    private static final class GroupCallHandler implements GroupProtocol, Upcall { 
	...
	private void handleInvocation(ReadMessage r) throws Exception { 
	    ...
	    case InvocationScheme.I_COMBINED_FLAT_GROUP: 
		{
		    ...
		    if (Group.DEBUG) {
			System.out.println(Group._rank + ": It is a GROUP INVOCATION");
	...
	public void upcall(ReadMessage m) { 
		...
		case INVOCATION:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got an INVOCATION");
		    handleInvocation(m);
		    break;
		...
		case INVOCATION_FLATCOMBINE:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got a INVOCATION_FLATCOMBINE");
		    stubIDStack[m.readInt()].handleFlatInvocationCombineMessage(m);
		    break;

		case INVOCATION_BINCOMBINE:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got a INVOCATION_BINCOMBINE");
		    stubIDStack[m.readInt()].handleBinInvocationCombineMessage(m);
		    break;

		case INVOCATION_REPLY:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got a INVOCATION_REPLY");

		    resultMode = m.readByte();
		    ticket = m.readInt();
		    int stub = ticket >> 16;
		    ticket = ticket & 0xFFFF;		
		    if (Group.DEBUG) System.out.println(Group._rank + ": INVOCATION_REPLY forwarded to stub (" + stub + ", " + ticket + ")");
		    stubIDStack[stub].handleResultMessage(m, ticket, resultMode);
		    break;

		case COMBINE:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got a COMBINE");
		    s = Group.getSkeleton(m.readInt());
		    s.handleCombineMessage(m);
		    break;

		case COMBINE_RESULT:
		    if (Group.DEBUG) System.out.println(Group._rank + ": Got a COMBINE_RESULT");
		    s = Group.getSkeletonByGroupID(m.readInt());
		    s.handleCombineMessage(m);
		    break;
		default: 
		    System.out.println(Group._rank + ": Got an illegal opcode !");
		} 
	    } catch (Exception e) { 
		System.out.println(Group._rank + ": Got an exception in GroupCallHandler !" + e);
		e.printStackTrace();
	    } 
	}
    } 
	...
	try {
		...
		systemIn  = portType.createReceivePort("GMI Client " + localID.name());
		systemIn.enableConnections();
     ...
     */
    public static void create(String nm, Class type, int size) throws RuntimeException {

	try { 
	    if (DEBUG) System.out.println(_rank + ": Group.create(" + nm + ", " + type + ", " + size + ") starting");
	    ...
	    if (DEBUG) System.out.println(_rank + ": Group.create(" + nm + ", " + size + ") waiting for reply on ticket(" + ticket +")");

	    RegistryReply r = (RegistryReply) ticketMaster.collect(ticket);

	    if (r.result == CREATE_FAILED) { 
		throw new RuntimeException(_rank + " Group.create(" + nm + ", " + size + ") Failed : Group already exists!");  
	    }

	    if (DEBUG) System.out.println(_rank + ": Group.create(" + nm + ", " + size + ") done");

	} catch (IOException e) { 
	    throw new RuntimeException(_rank + " Group.create(" + nm + ", " + size + ") Failed : communication error !" + e.getMessage());  
	}
    }
	...
	try { 
	    if (DEBUG) System.out.println(_rank + ": Group.join(" + nm + ", " + o + ") starting");
		...
		if (DEBUG) System.out.println(_rank + ": Group.join(" + nm + ") waiting for reply on ticket(" + ticket +")");
		
		r = (RegistryReply) ticketMaster.collect(ticket);
		
		switch(r.result) { 
		case JOIN_UNKNOWN: 
		    if (DEBUG) System.out.println(_rank + ": Group.join(" + nm + ") group not found, retry");
		    break;

		case JOIN_WRONG_TYPE:
		    throw new RuntimeException(_rank + " Group.join(" + nm + ") Failed : Group member has wrong type!");  

		case JOIN_FULL:
		    throw new RuntimeException(_rank + " Group.join(" + nm + ") Failed : Group full!");  
		...
		case JOIN_OK:
		    ...
		    break;
		default:
		    System.out.println(_rank + " Group.join(" + nm + ") Failed : got illegal opcode");  		
		    System.exit(1);
		} 
	    }
	    
	    if (DEBUG) System.out.println(_rank + ": Group.join(" + nm + ") group(" + groupnumber + ") found !");

	    o.init(groupnumber, memberRanks, memberSkels);			       
	    if (DEBUG) System.out.println(_rank + ": Group.join(" + nm + ", " + o + ") done");

	} catch (Exception e) { 
	    System.err.println("Got exception" + e);
	    e.printStackTrace();
	    throw new RuntimeException(_rank + " Group.join(" + nm + ") Failed : communication error !" + e.getMessage());  
	}
    } 
	...
	if (data == null) {
		...
		while (!done) { 
		    ...
		    switch (r.result) { 
		    case GROUP_UNKNOWN:
			throw new RuntimeException(Group._rank + " Group.lookup(" + nm + ") Failed : unknown group!");  	
			
		    case GROUP_NOT_READY: 
			System.err.println("Group " + nm + " not ready yet -> going to sleep");
			try { 
			    Thread.sleep(100);
			} catch (Exception e) { 
			    // ignore
		    ...
		    default:
			throw new RuntimeException(Group._rank + " Group.lookup(" + nm + ") Failed : unexpected answer!");  
		    } 
		} 
     ...
     */
    protected static CombinedInvocationInfo defineCombinedInvocation(CombinedInvocation ci, int groupID, String method, String nm, int mode, int rank, int size) throws RuntimeException { 
	try { 		
	    int ticket = ticketMaster.get();
	    WriteMessage w = unicast[0].newMessage();
	    w.writeByte(REGISTRY);
