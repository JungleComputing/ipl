...
implementation, but implementations based on native \todo{Platforms?
Perhaps better: `communication library'} platforms such as
...
Another often-used concept is the \emph{upcall}: a method invocation upon
an event such as the arrival of a message \todo{I explained 'upcall', but
you may want to change this.}. Upcalls can be used instead of downcalls
(explicit receipt, polling) in a number of cases.  These cases include
receiving messages, getting notifications of new connections, and updates
on new Ibis instances joining a computation.
...
\todo{This is a strange sentence. '... written using a write message'?? How
about `Once the send message has been constructed ...'?}
Once data is written using a write message at the send port, this
message will arrive at the receive port as a \emph{read message}. \todo{Next sentence is strange. Methods in the write message?? How about '.. to construct the write message'? or even '.. methods of the write message'?} There,
the data can be read again using methods equivalent to the ones in
the write message. Data must be read from the read message in the same order
...
\todo{Mention \texttt{java.io.Serializable} somewhere in one
of these paragraphs.}
...
\todo{
Why not
say that for each port the user must specify its capabilities,
and \emph{then} say that to make it easy to create similar ports
there is such a thing as a 'port type'. Makes it much easier to
understand why you need the notion of port type. Also,
you have introduced port types earlier in the document. Refer to
that? Put the motivation for port types (also) there?
}
...
result may be different. \todo{Do you mean two different Ibises
in the same run??}
