...
The Ibis registry also suppoers \emph{elections}, which can be used to
...
many-to-one communication is supported (in Ibis terms: multiple
sendports may connect to multiple receiveports).
...
requesting the receiption of a message from the receive port.
...
call the \texttt{poll} function in the receiveport to reliably receive
...
this purpose. It can be run by using the \texttx{ibisc}
