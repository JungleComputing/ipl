...
The port number provided must match the one given to ibis-server on start up 
using the \texttt{--port} option.

When additional hubs are started (see Section \ref{hubs}), their locations 
must also be provided to the ibis instances. This can be done using the using 
