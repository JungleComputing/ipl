 ...
 * of the Ibis compiler framework, and is the only means through which the Ibis
 * compiler framework communicates with the component.
 */
public abstract class IbiscComponent {
     ...
     *         otherwise.
     * @exception IllegalArgumentException may be thrown when there was an error in
     *                                     the arguments.
     */
    ...
    /**
     * This method processes an iterator which delivers java classes. The values are
    ...
    /**
     * Returns the name of the directory in which the file was found from which the
        ...
        if (fromIbisc) {
            for (int i = 1; i <= sz; i++) {
                ...
                for (String element : list) {
                    if (element.startsWith(fn) && element.endsWith(".class")) {
                        String name = base.getPath() + File.separator + element;
                        try {
