...
public class OrbTree {
    ...
    int checkLoad(int parent, int sum, int level) {
        ...
        if (parent < g.gdFirstLoc) load[parent] = 0;
        ...
        if (load[parent] > (int) ((1.0 + g.gdMaxLoad)
            * (double) (sum * (Orb[parent].last - Orb[parent].first + 1)) / (double) (g.Proc.ProcessorCount))) return (level + 1);
        else return (Math.max(lev0, lev1));
            ...
            for (i = 0; i < g.gdNumBodies; i++) {
                ...
                if ((pos.x < min.x) || (pos.x >= max.x) || (pos.y < min.y)
                    ...
                    for (j = 0; j < g.gdFirstLoc;) {
                        ...
                        if (pos.x >= Orb[j].upper.x || pos.y >= Orb[j].upper.y
                            || pos.z >= Orb[j].upper.z) j++;
                    }
    ...
    int findMedian(int pos[], int wei[], int medWei, int rootLoc) {
        ...
        int procs = Orb[rootLoc].last - Orb[rootLoc].first + 1, firstProc = Orb[rootLoc].first, loWei = 0, median = 0, actBodies = g.gdNumBodies, offset, size, subset[], shift, firstRad, locRad, partMed, i, j, k; // count is now private for this object
        ...
        for (shift = g.GD_MEDIAN_BITS - g.GD_MEDIAN_SHIFT; shift >= 0; shift -= g.GD_MEDIAN_SHIFT) {
            ...
            for (i = (g.Proc.myProc - firstProc + 1) % procs; i != g.Proc.myProc
                - firstProc; i = (i + 1) % procs) {

                firstRad = i * (g.GD_MEDIAN_RADIX / procs)
                    + Math.min(i, g.GD_MEDIAN_RADIX % procs);

                locRad = g.GD_MEDIAN_RADIX / procs
                    + ((i < g.GD_MEDIAN_RADIX % procs) ? 1 : 0);
            ...
            firstRad = (g.Proc.myProc - firstProc)
                * (g.GD_MEDIAN_RADIX / procs)
                + Math
                    .min(g.Proc.myProc - firstProc, g.GD_MEDIAN_RADIX % procs);

            locRad = g.GD_MEDIAN_RADIX
                / procs
                + (((g.Proc.myProc - firstProc) < g.GD_MEDIAN_RADIX % procs)
                    ? 1 : 0);
            ...
            for (i = 0; i < g.Proc.ProcessorCount; i++) {
                ...
                if (subset != null) {
                    ...
                    locRad = g.GD_MEDIAN_RADIX / procs
                        + ((j - firstProc < g.GD_MEDIAN_RADIX % procs) ? 1 : 0);
            ...
            if ((i > 0) && (count[i - 1] + loWei == medWei)) {
                ...
                for (j = g.GD_MEDIAN_RADIX - 1; (j >= 0)
                    && (count[j] + loWei >= medWei); j--)
                    ;
            ...
            if (partMed > 0) loWei += count[partMed - 1];
        }
    ...
    void LoadBalance(int level) {
        ...
        for (; level > 0; level--) {
            ...
            for (i = g.gdLogProcs, rootLoc = 0; i > level; i--) {
                ...
                if (g.Proc.myProc > Orb[rootLoc].last) rootLoc++;
            }
            ...
            for (i = 0; i < g.gdNumProcs; i++) {
                weights[i] = ((i >= firstProc) && (i < (firstProc + procs)))
                    ? totWeight : -1;
            }
            ...
            median = findMedian(pos, wei, totWeight * halfProcs / procs,
                rootLoc);

            newMedian = (procs != 1) ? (double) (median) / normFac + minPos
                : root.upper.element(dim);
            ...
            if (g.Proc.myProc < firstProc + halfProcs) {
                ...
                for (j = 0, i = g.gdNumBodies - 1; i >= 0; i--) {
                    ...
                    if (g.gdBodies[i].bPos.element(dim) >= newMedian) index[i] = firstProc
                        + halfProcs + (j++) % (procs - halfProcs);
                    else index[i] = g.Proc.myProc;
                }
            ...
            for (j = 0, i = 0; j < (1 << (g.gdLogProcs - level)); j++, i++) {
                ...
                normFac = (double) ((1 << g.GD_MEDIAN_BITS) - 1)
                    / t.element(dim);
                minPos = root.lower.element(dim);
    ...
    void ExchangeEssentialTree(BodyTree b) {
        ...
        for (int i = (g.Proc.myProc + 1) % g.Proc.ProcessorCount; i != g.Proc.myProc; i = (i + 1)
            % g.Proc.ProcessorCount) {
            sCOFM = 0;
            sBody = 0;
            iCount = 0;
    ...
    void sendEssential(BodyTreeNode btn, int dest, boolean inside) {
        ...
        if (t0x > 0) distSq += t0x * t0x;
        else if (t1x > 0) distSq += t1x * t1x;

        if (t0y > 0) distSq += t0y * t0y;
        else if (t1y > 0) distSq += t1y * t1y;

        if (t0z > 0) distSq += t0z * t0z;
        else if (t1z > 0) distSq += t1z * t1z;

        //    g.debugStr("distSQ: " + distSq + ", maxTheta: " + btn.getMaxTheta() ); 

        if (distSq >= btn.getMaxTheta()
            && (!btn.isLeaf() || btn.getNumBodies() > 0)) {

            // The node is far enough away; send the center of mass.

            //      g.debugStr("sendEssential, sending COFM; distSQ: " + distSq ); 

            CenterOfMass c = new CenterOfMass(btn.getMass(), btn.btnCenter_x,
                btn.btnCenter_y, btn.btnCenter_z, btn.btnCenterOfMass_x,
                btn.btnCenterOfMass_y, btn.btnCenterOfMass_z);
