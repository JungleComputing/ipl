...
final class NQueens extends SatinObject implements NQueensInterface,
    ...
    public long spawn_QueenNotInCorner(int[] board, int N, int spawnLevel,
        ...
        return QueenNotInCorner(board, N, spawnLevel, y, left, down, right,
            mask, lastmask, sidemask, bound1, bound2, topbit, endbit);
    }
    ...
    private static final long seq_QueenNotInCorner(final int[] board,
            final int N, final int y, final int left, final int down,
        ...
        if (y == N - 1) {
            if (bitmap != 0) {
                if ((bitmap & lastmask) == 0) {
                    board[y] = bitmap;
                    lnsol += Check(board, N, board[bound1], board[bound2],
                        topbit, endbit);
                    board[y] = 0;
                }
            }
        ...
        while (bitmap != 0) {
            ...
            board[y] = bit;
            bitmap ^= bit;
            lnsol += seq_QueenNotInCorner(board, N, y + 1, (left | bit) << 1,
    ...
    private long QueenNotInCorner(final int[] board, final int N,
        ...
        if (y > spawnLevel) {
            ...
            while (bitmap != 0) {
                ...
                board[y] = bit;
                bitmap ^= bit;
                lnsol += seq_QueenNotInCorner(board, N, y + 1,
                    (left | bit) << 1, down | bit, (right | bit) >> 1, mask,
    ...
    private static final long Check(final int[] board, final int N,
            final int board1, final int board2, final int topbit,
            final int endbit) {
    ...
    private final long calculate(final long[] results, final int[] bounds,
        ...
        for (int i = 0; i < bounds.length; i++) {
            ...
            if (SELECTED_BOUND == 0) {
                ...
                if (SELECTED_BOUND > 0 && SELECTED_BOUND <= SIZEE / 2
                    && SELECTED_BOUND <= SIZEE - SELECTED_BOUND) {
    ...
    private static void printResults(long[] results, int size, double time,
            boolean verbose) {
