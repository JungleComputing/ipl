...
class Raytracer { // implements java.io.Serializable {
    ...
    static void ReadDescriptor(StreamTokenizer d) throws IOException {
        if (d.ttype == StreamTokenizer.TT_EOF
                || d.ttype == StreamTokenizer.TT_EOL) {
        ...
        if (d.sval.equals("v")) {
            ...
            dummy.AddLight(new Light3D(dummy, ReadVector(d), 0.0001f,
                    new WhiteSurface(), World.DEFAULT_LIGHT_INTENSITY));
            ...
            Cone3D c = new Cone3D(dummy, current_surf, ReadVector(d),
                    read_double(d), ReadVector(d), read_double(d));
            dummy.AddObject(c);

        } else if (d.sval.equals("s")) {
            Sphere3D s = new Sphere3D(dummy, ReadVector(d), read_double(d),
                    current_surf);
            dummy.AddObject(s);
            ...
            Triangle3D t = new TrianglePatch3D(dummy, ReadVector(d),
                    ReadVector(d), ReadVector(d), ReadVector(d), ReadVector(d),
                    ReadVector(d), current_surf);
    ...
    public static void main(String arg[]) {
        ...
        for (int i = 0; i < arg.length; i++) {
            if (arg[i].equals("-dump")) {
                ...
                System.out
                        .println("Usage: java Raytracer [-dump] <input file>");
                System.exit(1);
            }
        }
        if (input == null) {
            input = "pics" + System.getProperty("file.separator")
                    + "balls2_small.nff";
            test = true;
        }
    ...
    public Raytracer(boolean dump, String input, boolean test) {
        ...
        System.out.println("application time raytrace (" + input + ") took "
                + time + " s");
