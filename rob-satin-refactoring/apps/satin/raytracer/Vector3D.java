...
class Vector3D implements java.io.Serializable {
    ...
    static Vector3D ReSize(Vector3D vec, double a) {
        return new Vector3D(vec.x * a, vec.y * a, vec.z * a);
    }
    ...
    static Vector3D Interpolate(Vector3D v1, Vector3D v2, double w1, double w2) {
        return new Vector3D((w1 * v1.x) + (w2 * v2.x), (w1 * v1.y)
                + (w2 * v2.y), (w1 * v1.z) + (w2 * v2.z));
    }

    static Vector3D Substract(Vector3D v1, Vector3D v2) {
    ...
    static Vector3D CrossProduct(Vector3D v1, Vector3D v2) {
        return new Vector3D((v1.y * v2.z) - (v1.z * v2.y), (v1.z * v2.x)
