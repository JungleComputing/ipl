...
public final class ClusterAwareRandomWorkStealing extends
    ...
    public InvocationRecord clientIteration() {
        ...
        synchronized (satin) {
            ...
            if (localVictim != null) {
                satin.lbComm.currentVictim = localVictim.getIdent();
            }
            if (!asyncStealInProgress) {
        ...
        if (remoteVictim != null && !asyncStealInProgress) {
            if (FT_NAIVE || canDoAsync) {
                ...
                try {
                    satin.lbComm.sendStealRequest(remoteVictim, false, false);
                } catch (IOException e) {
                    // Ignore this?
                }
            }
        ...
        // cluster)
        if (localVictim != null) {
            return satin.lbComm.stealJob(localVictim, false);
        }
