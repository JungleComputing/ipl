...
public final class SOCommunication implements Config, Protocol {
    ...
    public SendPort soSendPort;

    public PortType soPortType;

    /** used to do message combining on soSendPort */
    public MessageCombiner soMessageCombiner;
    ...
    public SOCommunication(Satin s) {
        this.s = s;
    }
    ...
    public void init(StaticProperties requestedProperties) {
        ...
        if (LABEL_ROUTING_MCAST) {
            ...
            new SOInvocationReceiver(s).start();
        }
    }

    public PortType createSOPortType(StaticProperties reqprops)
        ...
        for (int i = 0; i < joiners.length; i++) {
            ...
            try {
                ...
                rec = soPortType.createReceivePort("satin so receive port on "
                    ...
                    soInvocationHandler, s.ft.ftComm);
    ...
    public void sendAccumulatedSOInvocations() {
    ...
    public void fetchObject(String objectId, IbisIdentifier source)
        throws SOReferenceSourceCrashedException {
        // request the shared object from the source
        try {
            s.lbComm.currentVictim = source;
            Victim v;
            synchronized (s) {
        ...
        synchronized (s) {
            s.so.sharedObjects.put(s.so.object.objectId, s.so.object);
            s.so.object = null;
        }
    ...
    private void connectSOSendPort(IbisIdentifier ident) {
        ...
        // and connect
        if (Communication.connect(s.so.soComm.soSendPort, r,
