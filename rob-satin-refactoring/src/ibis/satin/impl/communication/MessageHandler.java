    ...
    public void upcall(ReadMessage m) {
        ...
        try {
            ...
            case BLOCKING_STEAL_REQUEST:
                ...
                s.lbComm.handleStealRequest(ident, opcode);
                break;
            ...
            case ASYNC_STEAL_REPLY_SUCCESS_TABLE:
                s.lbComm.handleReply(m, opcode);
                break;
            case JOB_RESULT_NORMAL:
            case JOB_RESULT_EXCEPTION:
                s.lbComm.handleJobResult(m, opcode);
                break;
            case ABORT:
                s.aborts.abortsComm.handleAbort(m);
                break;
            case ABORT_AND_STORE:
                s.ft.ftComm.handleAbortAndStore(m);
                break;
            case RESULT_REQUEST:
                s.ft.ftComm.handleResultRequest(m);
                break;
            case RESULT_PUSH:
                s.ft.ftComm.handleResultPush(m);
                break;
            case SO_REQUEST:
                s.so.soComm.handleSORequest(m);
                break;
            case SO_TRANSFER:
                s.so.soComm.handleSOTransfer(m);
                break;
            case BARRIER_REPLY:
