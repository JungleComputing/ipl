...
class UpcallSender implements MessageUpcall {
    ...
    public void start() {
        try {
            ...
            WriteMessage writeMessage = sport.newMessage();
            if (buffer != null) {
            	System.err.println("Writing message" + count);
                writeMessage.writeArray(buffer);
            }
            writeMessage.finish();
...
//            		PortType.SERIALIZATION_DATA,
            ...
            p.setProperty("ibis.verbose", "true");
            p.setProperty("ibis.pool.name", "Benchmark");
    		p.setProperty("ibis.server.address","localhost");
    		p.setProperty("ibis.implementation", "sns,smartsockets");
    		p.setProperty("ibis.upcall.communication", "true");
    		p.setProperty("ibis.upcall.connections", "true");
    		...
    		for (Enumeration propertyNames = snsProperties.propertyNames(); propertyNames.hasMoreElements(); )
    		{
    			...
    			p.put(key, snsProperties.get(key));
    		}    	      
   		
    		class EventHandler implements RegistryEventHandler {
				@Override
    		...
    		ibis = IbisFactory.createIbis(s, p, false, new EventHandler(), t);
            ...
            if (rank == 0) {
                ...
                if (!upcallsend) {
                    ...
                    rport.enableConnections();
                    logger.debug("LAT: Connect sendport to " + remote.toString());
                    sport.connect(remote, "test port");
                    rport.enableMessageUpcalls();
                    logger.debug("LAT: Starting UpcallSender");
                    sender.start();
                    sender.finish();
                    logger.debug("LAT: UpcallSender Finished");
                }
            } else {
            	logger.debug("LAT: Connect sendport to " + remote.toString());
                sport.connect(remote, "test port");
