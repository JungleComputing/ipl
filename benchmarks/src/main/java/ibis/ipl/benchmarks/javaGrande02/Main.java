...
final class Main {
    ...
    private static long one_way(ReceivePort rport, SendPort sport, int count, Object data, boolean stream) throws Exception {
        ...
        if (verbose) {
            System.out.println("One way test took " + (end - start) + " milliseconds");
        }
    ...
    private static long two_way(ReceivePort rport, SendPort sport, int count, Object data) throws Exception {
        ...
        if (verbose) {
            System.out.println("Two way test took " + (end - start) + " milliseconds");
        }
        return (end - start);
    }

    private static long runTest(ReceivePort rport, SendPort sport, int count, int retries, Object data, boolean one_way, boolean stream)
            throws Exception {
        ...
        if (verbose) {
            System.out.println("Best time : " + best + " milliseconds");
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            PortType t = new PortType(PortType.COMMUNICATION_RELIABLE, PortType.CONNECTION_ONE_TO_ONE, PortType.RECEIVE_AUTO_UPCALLS,
                    PortType.RECEIVE_EXPLICIT, ibisSer ? PortType.SERIALIZATION_OBJECT_IBIS : PortType.SERIALIZATION_OBJECT_SUN);
            ...
            if (master.equals(ibis.identifier())) {
                ...
                if (array) {
                    ...
                    System.out
                            .println("byte[" + arraysize + "] = " + tp((one_way ? arraysize * count : 2 * arraysize * count), time) + " MBytes/sec.");
                ...
                if (dlist) {
                    ...
                    time = runTest(rport, sport, count, retries, data, one_way, stream);
                    System.out
                            .println("dlist(" + len + ") = " + tp((one_way ? len * count : 2 * len * count) * DList.PAYLOAD, time) + " MBytes/sec.");
                }
                ...
                if (oarray) {
                    ...
                    System.out
                            .println("object[" + len + "] = " + tp((one_way ? len * count : 2 * len * count) * Data.PAYLOAD, time) + " MBytes/sec.");
                }
