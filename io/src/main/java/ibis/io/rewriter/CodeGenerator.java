...
class CodeGenerator implements RewriterConstants {
    ...
    private void versionUID() {
        for (Field f : fields) {
            if (f.getName().equals(FIELD_SERIAL_VERSION_UID) && f.isFinal() && f.isStatic()) {
        ...
        if (uid != 0) {
            FieldGen f = new FieldGen(Const.ACC_PRIVATE | Const.ACC_FINAL | Const.ACC_STATIC, Type.LONG, FIELD_SERIAL_VERSION_UID, constantpool);
            f.setInitValue(uid);
    ...
    private Instruction createGeneratedWriteObjectInvocation(String name, short invmode) {
        return factory.createInvoke(name, METHOD_GENERATED_WRITE_OBJECT, Type.VOID, ibis_output_stream_arrtp, invmode);
    }

    private Instruction createGeneratedDefaultReadObjectInvocation(String name, InstructionFactory fac, short invmode) {
        return fac.createInvoke(name, METHOD_GENERATED_DEFAULT_READ_OBJECT, Type.VOID, new Type[] { ibis_input_stream, Type.INT }, invmode);
    }

    private Instruction createInitInvocation(String name, InstructionFactory f) {
        return f.createInvoke(name, METHOD_INIT, Type.VOID, ibis_input_stream_arrtp, Const.INVOKESPECIAL);
    }

    private Instruction createGeneratedDefaultWriteObjectInvocation(String name) {
        return factory.createInvoke(name, METHOD_GENERATED_DEFAULT_WRITE_OBJECT, Type.VOID, new Type[] { ibis_output_stream, Type.INT },
                Const.INVOKESPECIAL);
    }

    private Instruction createWriteObjectInvocation() {
        return factory.createInvoke(classname, METHOD_WRITE_OBJECT, Type.VOID, new Type[] { sun_output_stream }, Const.INVOKESPECIAL);
    }
    ...
    void generateEmptyMethods() {
        ...
        if (generator.isVerbose()) {
            ...
            System.out.println("    " + classname + " implements java.io.Serializable -> adding " + TYPE_IBIS_IO_SERIALIZABLE);
        }
        ...
        MethodGen write_method = new MethodGen(flags, Type.VOID, ibis_output_stream_arrtp, new String[] { VARIABLE_OUTPUT_STREAM },
        ...
        MethodGen default_write_method = new MethodGen(flags, Type.VOID, new Type[] { ibis_output_stream, Type.INT },
                new String[] { VARIABLE_OUTPUT_STREAM, VARIABLE_LEVEL }, METHOD_GENERATED_DEFAULT_WRITE_OBJECT, classname, il, constantpool);
        ...
        MethodGen default_read_method = new MethodGen(flags, Type.VOID, new Type[] { ibis_input_stream, Type.INT },
                new String[] { VARIABLE_OUTPUT_STREAM, VARIABLE_LEVEL }, METHOD_GENERATED_DEFAULT_READ_OBJECT, classname, il, constantpool);

        default_read_method.addException(TYPE_JAVA_IO_IOEXCEPTION);
        default_read_method.addException(TYPE_JAVA_LANG_CLASS_NOT_FOUND_EXCEPTION);
        ...
        if (is_externalizable || !super_is_serializable || generator.forceGeneratedCalls() || super_has_ibis_constructor) {
            ...
            MethodGen read_cons = new MethodGen(Const.ACC_PUBLIC, Type.VOID, ibis_input_stream_arrtp, new String[] { VARIABLE_INPUT_STREAM },
                    METHOD_INIT, classname, il, constantpool);
            ...
            il = new InstructionList();
            il.append(new RETURN());
            MethodGen readobjectWrapper = new MethodGen(Const.ACC_PUBLIC, Type.VOID, ibis_input_stream_arrtp, new String[] { VARIABLE_INPUT_STREAM },
                    METHOD_$READ_OBJECT_WRAPPER$, classname, il, constantpool);
            readobjectWrapper.addException(TYPE_JAVA_IO_IOEXCEPTION);
            readobjectWrapper.addException(TYPE_JAVA_LANG_CLASS_NOT_FOUND_EXCEPTION);
    ...
    private InstructionList writeInstructions(Field field) {
        ...
        temp.append(factory.createFieldAccess(classname, field.getName(), t, Const.GETFIELD));
        temp.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, info.write_name, Type.VOID, info.param_tp_arr,
                Const.INVOKEVIRTUAL));
    ...
    private InstructionList readInstructions(Field field, boolean from_constructor) {
        ...
        if (from_constructor || !field.isFinal()) {
            ...
            temp.append(new ALOAD(1));
            temp.append(
                    factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, info.read_name, info.tp, Type.NO_ARGS, Const.INVOKEVIRTUAL));

            if (!info.primitive) {
                temp.append(factory.createCheckCast((ReferenceType) t));
            }

            temp.append(factory.createFieldAccess(classname, field.getName(), t, Const.PUTFIELD));
        } else {
            temp.append(new ALOAD(1));
            temp.append(new ALOAD(0));
            ...
            temp.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, info.final_read_name, Type.VOID,
                    info.primitive ? new Type[] { Type.OBJECT, Type.STRING, Type.STRING }
    ...
    private InstructionList writeReferenceField(Field field) {
        ...
        if (field_type instanceof ObjectType) {
            ...
            if (el_type instanceof ObjectType) {
                field_class = lookupClass(((ObjectType) el_type).getClassName());
                if (field_class != null && field_class.isFinal()) {
                    isfinal = true;
                }
        ...
        if ((basicname != null) || (isfinal && (SerializationInfo.hasIbisConstructor(field_class)
                || (SerializationInfo.isSerializable(field_class) && generator.forceGeneratedCalls())))) {
            ...
            write_il.append(factory.createFieldAccess(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, VARIABLE_REPLACER,
            ...
            write_il.append(factory.createFieldAccess(classname, field.getName(), field_type, Const.GETFIELD));
            if (basicname != null) {
                write_il.append(factory.createFieldAccess(TYPE_IBIS_IO_CONSTANTS, "TYPE_" + basicname.toUpperCase(), Type.INT, Const.GETSTATIC));
                write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_WRITE_KNOWN_ARRAY_HEADER, Type.INT,
                        new Type[] { Type.OBJECT, Type.INT }, Const.INVOKEVIRTUAL));
            } else {
                write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_WRITE_KNOWN_OBJECT_HEADER, Type.INT,
            ...
            if (isarray) {
                ...
                write_il.append(new ISTORE(3));
                write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_WRITE_INT, Type.VOID,
                ...
                if (basicname != null) {
                    ...
                    write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, writeCallName(basicname), Type.VOID,
                            new Type[] { field_type, Type.INT, Type.INT }, Const.INVOKEVIRTUAL));
                } else {
                    write_il.append(new ICONST(0));
                    write_il.append(new ISTORE(4));
                    ...
                    write_il.append(new AALOAD());
                    write_il.append(new ALOAD(1));
                    write_il.append(createGeneratedWriteObjectInvocation(field_class.getClassName(), Const.INVOKEVIRTUAL));
    ...
    private InstructionList readReferenceField(Field field, boolean from_constructor) {
        ...
        if ((basicname != null) || (isfinal && (SerializationInfo.hasIbisConstructor(field_class)
            ...
            read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_KNOWN_TYPE_HEADER, Type.INT, Type.NO_ARGS,
                    Const.INVOKEVIRTUAL));
            ...
            if (isarray) {
                if (basicname != null) {
                    ...
                    read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, callname, field_type, Type.NO_ARGS,
                            Const.INVOKEVIRTUAL));
                    read_il.append(factory.createFieldAccess(classname, field.getName(), field_type, Const.PUTFIELD));
                } else {
                    Type el_type = ((ArrayType) field_type).getElementType();
                    read_il.append(new ALOAD(0));
                    read_il.append(new ALOAD(1));
                    read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_INT, Type.INT, Type.NO_ARGS,
                            Const.INVOKEVIRTUAL));
                    ...
                    read_il.append(new ALOAD(1));
                    read_il.append(new ALOAD(0));
                    read_il.append(factory.createFieldAccess(classname, field.getName(), field_type, Const.GETFIELD));

                    read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_ADD_OBJECT_TO_CYCLE_CHECK, Type.VOID,
                            new Type[] { Type.OBJECT }, Const.INVOKEVIRTUAL));
                    ...
                    read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_KNOWN_TYPE_HEADER, Type.INT,
                    ...
                    read_il.append(new ALOAD(1));
                    read_il.append(new ILOAD(2));
                    read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_GET_OBJECT_FROM_CYCLE_CHECK, Type.OBJECT,
                            new Type[] { Type.INT }, Const.INVOKEVIRTUAL));
    ...
    private InstructionHandle generateReadField(String tpname, Type tp, InstructionList read_il, GOTO gto, boolean from_constructor) {
        ...
        if (from_constructor || !has_final_fields) {
            ...
            read_il.append(new ALOAD(1));
            if (tpname.equals("")) {
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_OBJECT, Type.OBJECT, Type.NO_ARGS,
                        Const.INVOKEVIRTUAL));
            } else {
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ + tpname, tp, Type.NO_ARGS,
                        Const.INVOKEVIRTUAL));
            }
            read_il.append(factory.createInvoke(TYPE_JAVA_LANG_REFLECT_FIELD, METHOD_SET + tpname, Type.VOID, new Type[] { Type.OBJECT, tp },
                    Const.INVOKEVIRTUAL));
            read_il.append(gto);
        ...
        if (tpname.equals("")) {
            ...
            read_il.append(factory.createInvoke(TYPE_JAVA_LANG_REFLECT_FIELD, METHOD_GET_TYPE, new ObjectType(TYPE_JAVA_LANG_CLASS), Type.NO_ARGS,
                    Const.INVOKEVIRTUAL));
            read_il.append(factory.createInvoke(TYPE_JAVA_LANG_CLASS, METHOD_GET_NAME, Type.STRING, Type.NO_ARGS, Const.INVOKEVIRTUAL));

            read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_FIELD_OBJECT, Type.VOID,
                    new Type[] { Type.OBJECT, Type.STRING, Type.STRING }, Const.INVOKEVIRTUAL));
        } else {
            read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_FIELD + tpname, Type.VOID,
                    new Type[] { Type.OBJECT, Type.STRING }, Const.INVOKEVIRTUAL));
        }
        GOTO gto2 = new GOTO(null);
        read_il.append(gto2);
        ...
        if (tpname.equals("")) {
            read_il.append(
                    factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_OBJECT, tp, Type.NO_ARGS, Const.INVOKEVIRTUAL));
        } else {
            read_il.append(
                    factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ + tpname, tp, Type.NO_ARGS, Const.INVOKEVIRTUAL));
        }
        read_il.append(factory.createInvoke(TYPE_JAVA_LANG_REFLECT_FIELD, METHOD_SET + tpname, Type.VOID, new Type[] { Type.OBJECT, tp },
                Const.INVOKEVIRTUAL));
    ...
    private JavaClass generateInstanceGenerator() {
        ...
        if (generator.isVerbose()) {
            System.out.println("  Generating InstanceGenerator class for " + classname);
        }
        ...
        ClassGen iogenGen = new ClassGen(name, TYPE_IBIS_IO_GENERATOR, classfilename, Const.ACC_FINAL | Const.ACC_PUBLIC | Const.ACC_SUPER, null);
        ...
        if (!is_externalizable && super_is_serializable && !super_has_ibis_constructor && !generator.forceGeneratedCalls()) {
            ...
            il.append(iogenFactory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_CREATE_UNINITIALIZED_OBJECT, Type.OBJECT,
            ...
            il.append(new LDC(ind));
            il.append(iogenFactory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_READ_SERIALIZABLE_OBJECT, Type.VOID,
                    new Type[] { Type.OBJECT, Type.STRING }, Const.INVOKEVIRTUAL));
            ...
            if (SerializationInfo.hasReadObject(methods)) {
                ...
                il.append(new ALOAD(1));
                il.append(
                        iogenFactory.createInvoke(classname, METHOD_$READ_OBJECT_WRAPPER$, Type.VOID, ibis_input_stream_arrtp, Const.INVOKEVIRTUAL));
            } else {
                ...
                il.append(createGeneratedDefaultReadObjectInvocation(classname, iogenFactory, Const.INVOKEVIRTUAL));
            }
            il.append(new ALOAD(2));
        } else {
        ...
        MethodGen method = new MethodGen(Const.ACC_FINAL | Const.ACC_PUBLIC, Type.OBJECT, ibis_input_stream_arrtp,
                new String[] { VARIABLE_INPUT_STREAM }, METHOD_GENERATED_NEW_INSTANCE, name, il, iogenGen.getConstantPool());
        ...
        il.append(iogenFactory.createInvoke(TYPE_IBIS_IO_GENERATOR, METHOD_INIT, Type.VOID, Type.NO_ARGS, Const.INVOKESPECIAL));
        il.append(new RETURN());

        method = new MethodGen(Const.ACC_PUBLIC, Type.VOID, Type.NO_ARGS, null, METHOD_INIT, name, il, iogenGen.getConstantPool());
    ...
    private void fillInGeneratedReadObjectMethod(int dpth) {
        ...
        if (is_externalizable || super_has_ibis_constructor || !super_is_serializable || generator.forceGeneratedCalls()) {
            ...
            if (is_externalizable) {
                read_il.append(new ALOAD(0));
                read_il.append(factory.createInvoke(classname, METHOD_INIT, Type.VOID, Type.NO_ARGS, Const.INVOKESPECIAL));
            } else if (!super_is_serializable) {
                read_il.append(new ALOAD(0));
                read_il.append(factory.createInvoke(super_classname, METHOD_INIT, Type.VOID, Type.NO_ARGS, Const.INVOKESPECIAL));
            } else {
                read_il.append(new ALOAD(0));
                read_il.append(new ALOAD(1));
            ...
            int read_cons_index = SerializationInfo.findMethod(methods, METHOD_INIT, SIGNATURE_LIBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM_V);
            mgen = new MethodGen(methods[read_cons_index], classname, constantpool);
            index = read_cons_index;
        } else if (SerializationInfo.hasReadObject(methods)) {
            int read_wrapper_index = SerializationInfo.findMethod(methods, METHOD_$READ_OBJECT_WRAPPER$,
        ...
        /*
         * TODO: Shouldn't there be an else clause here that fills in a method? Even an
         * exception throwing method might be better? There is almost certainly a
         * problem here in some corner case.
         */
        ...
        if (read_il != null) {
            ...
            if (is_externalizable || SerializationInfo.hasReadObject(methods)) {
                ...
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_PUSH_CURRENT_OBJECT, Type.VOID,
                        new Type[] { Type.OBJECT, Type.INT }, Const.INVOKEVIRTUAL));

                read_il.append(new ALOAD(0));
                read_il.append(new ALOAD(1));
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_GET_JAVA_OBJECT_INPUT_STREAM,
                        sun_input_stream, Type.NO_ARGS, Const.INVOKEVIRTUAL));
                if (is_externalizable) {
                    /* Invoke readExternal */
                    read_il.append(factory.createInvoke(classname, METHOD_READ_EXTERNAL, Type.VOID,
                            new Type[] { new ObjectType(TYPE_JAVA_IO_OBJECT_INPUT) }, Const.INVOKEVIRTUAL));
                } else {
                    /* Invoke readObject. */
                    read_il.append(
                            factory.createInvoke(classname, METHOD_READ_OBJECT, Type.VOID, new Type[] { sun_input_stream }, Const.INVOKESPECIAL));
                }
                 ...
                 */
                read_il.append(new ALOAD(1));
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_POP_CURRENT_OBJECT, Type.VOID, Type.NO_ARGS,
                        Const.INVOKEVIRTUAL));
            } else {
            ...
            if (is_externalizable || !super_is_serializable || generator.forceGeneratedCalls() || super_has_ibis_constructor) {
                newMethod = new MethodGen(Const.ACC_PUBLIC, Type.VOID, ibis_input_stream_arrtp, new String[] { VARIABLE_INPUT_STREAM }, METHOD_INIT,
                        classname, read_il, constantpool);
            } else if (SerializationInfo.hasReadObject(methods)) {
                newMethod = new MethodGen(Const.ACC_PUBLIC, Type.VOID, ibis_input_stream_arrtp, new String[] { VARIABLE_INPUT_STREAM },
                        METHOD_$READ_OBJECT_WRAPPER$, classname, read_il, constantpool);
            }
            newMethod.addException(TYPE_JAVA_IO_IOEXCEPTION);
            newMethod.addException(TYPE_JAVA_LANG_CLASS_NOT_FOUND_EXCEPTION);

            newMethod.setMaxStack(MethodGen.getMaxStack(constantpool, read_il, newMethod.getExceptionHandlers()));
    ...
    private void fillInGeneratedWriteObjectMethod(int dpth) {
        ...
        int write_method_index = SerializationInfo.findMethod(methods, METHOD_GENERATED_WRITE_OBJECT,
                SIGNATURE_LIBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM_V);

        MethodGen write_gen = new MethodGen(methods[write_method_index], classname, constantpool);
        ...
        if (is_externalizable) {
            ...
            write_il.append(new ALOAD(0));
            write_il.append(new ALOAD(1));
            write_il.append(createGeneratedWriteObjectInvocation(super_classname, Const.INVOKESPECIAL));
            ...
            write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_WRITE_SERIALIZABLE_OBJECT, Type.VOID,
                    new Type[] { Type.OBJECT, Type.STRING }, Const.INVOKEVIRTUAL));
        }
        ...
        if (is_externalizable || SerializationInfo.hasWriteObject(methods)) {
            ...
            write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_PUSH_CURRENT_OBJECT, Type.VOID,
                    new Type[] { Type.OBJECT, Type.INT }, Const.INVOKEVIRTUAL));

            write_il.append(new ALOAD(0));
            write_il.append(new ALOAD(1));
            write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_GET_JAVA_OBJECT_OUTPUT_STREAM,
                    sun_output_stream, Type.NO_ARGS, Const.INVOKEVIRTUAL));
            if (is_externalizable) {
                /* Invoke writeExternal */
                write_il.append(factory.createInvoke(classname, METHOD_WRITE_EXTERNAL, Type.VOID,
                        new Type[] { new ObjectType(TYPE_JAVA_IO_OBJECT_OUTPUT) }, Const.INVOKEVIRTUAL));
            } else {
                /* Invoke writeObject. */
             ...
             */
            write_il.append(new ALOAD(1));
            write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_POP_CURRENT_OBJECT, Type.VOID, Type.NO_ARGS,
                    Const.INVOKEVIRTUAL));
        } else {
        ...
        MethodGen new_write_gen = new MethodGen(flags, Type.VOID, ibis_output_stream_arrtp, new String[] { VARIABLE_OUTPUT_STREAM },
        ...
        new_write_gen.setMaxStack(MethodGen.getMaxStack(constantpool, write_il, write_gen.getExceptionHandlers()));
    ...
    private void fillInGeneratedDefaultWriteObjectMethod(int dpth) {
        ...
        int default_write_method_index = SerializationInfo.findMethod(methods, METHOD_GENERATED_DEFAULT_WRITE_OBJECT,
        ...
        if (super_is_ibis_serializable || super_is_serializable) {
            ...
            if (super_is_ibis_serializable || generator.forceGeneratedCalls()) {
                ...
                write_il.append(new ALOAD(0));
                write_il.append(new ILOAD(2));
                write_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_OUTPUT_STREAM, METHOD_DEFAULT_WRITE_SERIALIZABLE_OBJECT,
                        Type.VOID, new Type[] { Type.OBJECT, Type.INT }, Const.INVOKEVIRTUAL));
            }
        } else {
        ...
        int flags = Const.ACC_PUBLIC | (gen.isFinal() ? Const.ACC_FINAL : 0);
        MethodGen default_write_method = new MethodGen(flags, Type.VOID, new Type[] { ibis_output_stream, Type.INT },
                new String[] { VARIABLE_OUTPUT_STREAM, VARIABLE_LEVEL }, METHOD_GENERATED_DEFAULT_WRITE_OBJECT, classname, write_il, constantpool);

        default_write_method.addException(TYPE_JAVA_IO_IOEXCEPTION);

        default_write_method.setMaxStack(MethodGen.getMaxStack(constantpool, write_il, default_write_method.getExceptionHandlers()));
    ...
    private void fillInGeneratedDefaultReadObjectMethod(int dpth) {
        ...
        IF_ICMPNE ifcmpne;
        int default_read_method_index = SerializationInfo.findMethod(methods, METHOD_GENERATED_DEFAULT_READ_OBJECT,
        ...
        if (super_is_ibis_serializable || super_is_serializable) {
            ...
            if (super_is_ibis_serializable || generator.forceGeneratedCalls()) {
                ...
                read_il.append(new ALOAD(1));
                read_il.append(new ILOAD(2));
                read_il.append(createGeneratedDefaultReadObjectInvocation(super_classname, factory, Const.INVOKESPECIAL));
            } else {
                /*
                 * Superclass is not rewritten.
                 */
                ...
                read_il.append(new ALOAD(0));
                read_il.append(new ILOAD(2));
                read_il.append(factory.createInvoke(TYPE_IBIS_IO_IBIS_SERIALIZATION_INPUT_STREAM, METHOD_DEFAULT_READ_SERIALIZABLE_OBJECT, Type.VOID,
                        new Type[] { Type.OBJECT, Type.INT }, Const.INVOKEVIRTUAL));
            }
        } else {
            ifcmpne.setTarget(end);
        ...
        MethodGen default_read_method = new MethodGen(flags, Type.VOID, new Type[] { ibis_input_stream, Type.INT },
                new String[] { VARIABLE_OUTPUT_STREAM, VARIABLE_LEVEL }, METHOD_GENERATED_DEFAULT_READ_OBJECT, classname, read_il, constantpool);
        ...
        default_read_method.setMaxStack(MethodGen.getMaxStack(constantpool, read_il, default_read_method.getExceptionHandlers()));
    ...
    private void generateStackMap(MethodGen m) {
        ...
        ArrayList<StackMapEntry> entries = new ArrayList<>();
        int oldnLocals = nLocals;
        boolean first = true;
        for (int position : positions) {
            int pos = position - prevPosition;
            while (localStart[nLocals] != null && localStart[nLocals].getPosition() < position) {
                nLocals++;
            }
            if (nLocals > oldnLocals) {
                if (first && m.getName().equals("<init>")) {
                    ...
                    entries.add(new StackMapEntry(Const.FULL_FRAME, pos, types, null, null));
                    size += 7;
                } else {
                    ...
                    entries.add(new StackMapEntry(Const.APPEND_FRAME + (nLocals - oldnLocals - 1), pos, types, null, null));
                    size += 3;
                }
            } else if (pos < 64) {
                // SAME
            ...
            prevPosition = position + 1; // yes: + 1. See
                                         // http://http://docs.oracle.com/javase/specs/jvms/se7/html/jvms-4.html#jvms-4.7.4
            oldnLocals = nLocals;
        }
