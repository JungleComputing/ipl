...
public class SimpleBigConversion extends Conversion {
    ...
    public void byte2int(byte[] src, int index_src, int[] dst, int index_dst, int len) {
        ...
        for (int i = 0; i < len; i++) {
            dst[index_dst + i] = (((src[count + 3] & 0xff) << 0) | ((src[count + 2] & 0xff) << 8) | ((src[count + 1] & 0xff) << 16)
                    | ((src[count + 0] & 0xff) << 24));
            count += 4;
        }
    }
    ...
    public void byte2long(byte[] src, int index_src, long[] dst, int index_dst, int len) {
        ...
        for (int i = index_dst; i < end; i++) {
            int t1 = (((src[count + 0] & 0xff) << 24) | ((src[count + 1] & 0xff) << 16) | ((src[count + 2] & 0xff) << 8)
                    | ((src[count + 3] & 0xff) << 0));
            int t2 = (((src[count + 4] & 0xff) << 24) | ((src[count + 5] & 0xff) << 16) | ((src[count + 6] & 0xff) << 8)
                    | ((src[count + 7] & 0xff) << 0));
    ...
    public void byte2float(byte[] src, int index_src, float[] dst, int index_dst, int len) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            temp = (((src[count + 0] & 0xff) << 24) | ((src[count + 1] & 0xff) << 16) | ((src[count + 2] & 0xff) << 8)
                    | ((src[count + 3] & 0xff) << 0));
