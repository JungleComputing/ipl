...
/**
 * The <code>SerializationOutput</code> interface specifies which methods must
 * be implemented by any serialization output stream type. Some of these methods
 * may just throw an exception, depending on the serialization type. For
 * instance, the <code>writeInt</code> method of a "byte" serialization output
 * stream will just throw an exception.
 * <p>
 * For all write methods in this interface, the invariant is that the reads must
 * match the writes one by one. The only exception to this rule is that an array
 * written with any of the <code>writeArray</code> methods can be read by
 * {@link SerializationInput#readObject SerializationInput.readObject}. <strong>
 * In particular, an array written with {@link #writeObject writeObject} cannot
 * be read with <code>readArray</code>, because {@link #writeObject writeObject}
...
public interface SerializationOutput extends DataOutput {
    ...
    /**
     * Writes a <code>String</code> to the output. A duplicate check for this
     * <code>String</code> object is performed: if the object was already written to
     * this message, a handle for this object is written instead of the object
     * itself.
     *
    ...
    /**
     * Writes a <code>Serializable</code> object to the output. A duplicate check
     * for this <code>String</code> object is performed: if the object was already
     * written to this output, a handle for this object is written instead of the
     * object itself.
     *
     ...
     *
     * @exception java.io.IOException is thrown when the implementation refuses to
     *                                set the replacer, for instance when it is not
