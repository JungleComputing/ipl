...
final class AlternativeTypeInfo {
    ...
    private Method getMethod(String name, Class<?>[] paramTypes, Class<?> returnType) {
        ...
        if ((method.getReturnType() != returnType) || ((method.getModifiers() & Modifier.STATIC) != 0)) {
            return null;
        }
    ...
    private AlternativeTypeInfo(Class<?> clazz) {
        ...
        try {
            ...
            for (Class<?> intf : intfs) {
                if (intf.equals(ibis.io.Serializable.class)) {
            ...
            if (serial_persistent_fields == null) {
                ...
                for (Field field : fields) {
                    ...
                    if ((modifiers & (Modifier.TRANSIENT | Modifier.STATIC)) == 0) {
                        ...
                        try {
                            ...
                            if (logger.isInfoEnabled()) {
                                logger.info("Cannot set field " + field.getName() + " to accessible for class " + clazz.getName() + ": " + e);
                            }
                        }
                ...
                for (ObjectStreamField serial_persistent_field : serial_persistent_fields) {
                    Field field = findField(serial_persistent_field);
                    Class<?> field_type = serial_persistent_field.getType();
                    if (field != null) {
                        try {
                            field.setAccessible(true);
                        } catch (Exception e) {
    ...
    int getOffset(String name, Class<?> tp) throws IllegalArgumentException {
        ...
        if (tp.isPrimitive()) {
            if (serial_persistent_fields != null) {
                for (ObjectStreamField serial_persistent_field : serial_persistent_fields) {
                    if (serial_persistent_field.getType() == tp) {
                ...
                for (Field serializable_field : serializable_fields) {
                    if (serializable_field.getType() == tp) {
