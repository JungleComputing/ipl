...
final class AlternativeTypeInfo {
	...
	/**
	 * Maintains all <code>AlternativeTypeInfo</code> structures in a hashmap, to be
	 * accessed through their classname.
	 */
	...
	private static class ArrayWriter extends IbisWriter {
		@Override
		void writeObject(IbisSerializationOutputStream out, Object ref, AlternativeTypeInfo t, int hashCode,
	...
	private static class ExternalizableWriter extends IbisWriter {
		...
		void writeObject(IbisSerializationOutputStream out, Object ref, AlternativeTypeInfo t, int hashCode,
			...
			((java.io.Externalizable) ref).writeExternal(out.getJavaObjectOutputStream());
	...
	private static class IbisSerializableReader extends IbisReader {
		@Override
		Object readObject(IbisSerializationInputStream in, AlternativeTypeInfo t, int typeHandle)
		...
		Object readObject(IbisSerializationInputStream in, AlternativeTypeInfo t, int typeHandle)
			...
			while (!clazz.isEnum()) {
				...
				if (clazz == null || clazz == java.lang.Object.class) {
					throw new IOException("Exception while reading enumeration: class is not an enumeration");
				}
			}
			try {
	...
	private static class ExternalizableReader extends IbisReader {
		...
		Object readObject(IbisSerializationInputStream in, AlternativeTypeInfo t, int typeHandle)
			...
			((java.io.Externalizable) obj).readExternal(in.getJavaObjectInputStream());
	...
	/**
	 * For each field, indicates whether the field is final. This is significant for
	 * deserialization, because it determines the way in which the field can be
	 * assigned to. The bytecode verifier does not allow arbitraty assignments to
	 * final fields.
	 */
	...
	/**
	 * The "level" of a serializable class. The "level" of a serializable class is
	 * computed as follows: - if its superclass is serializable: the level of the
	 * superclass + 1. - if its superclass is not serializable: 1.
	 */
	int level;
	 ...
	 * non-serializable class in the type hierarchy needs to have a parameter-less
	 * constructor that is visible from this class.
	 *
	...
	private Method getMethod(String name, Class<?>[] paramTypes, Class<?> returnType) {
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("Cannot set method " + method.getName() + " to accessible");
			}
		}
		return method;
	}
	 ...
	 *
	 * @param o   the object on which <code>writeObject</code> is to be invoked
	 * @param out the <code>ObjectOutputStream</code> to be given as parameter
	 * @exception IOException when anything goes wrong
	 */
	 ...
	 *
	 * @param o  the object on which <code>readObject</code> is to be invoked
	 * @param in the <code>ObjectInputStream</code> to be given as parameter
	 * @exception IOException when anything goes wrong
	 */
	 ...
	 *
	 * @param o the object on which <code>readResolve</code> is to be invoked
	 * @exception IOException when anything goes wrong
	 */
	 ...
	 *
	 * @param o the object on which <code>writeReplace</code> is to be invoked
	 * @exception IOException when anything goes wrong
	 */
	...
	/**
	 * Constructor is private. Use {@link #getAlternativeTypeInfo(Class)} to obtain
	 * the <code>AlternativeTypeInfo</code> for a type.
	 */
	private AlternativeTypeInfo(Class<?> clazz) {
		...
		try {
			/*
			 * Here we figure out what field the type contains, and which fields we should
			 * write. We must also sort them by type and name to ensure that we read them
			 * correctly on the other side. We cache all of this so we only do it once for
			 * each type.
			 */
			...
			writeObjectMethod = getMethod("writeObject", new Class<?>[] { ObjectOutputStream.class }, Void.TYPE);
			readObjectMethod = getMethod("readObject", new Class<?>[] { ObjectInputStream.class }, Void.TYPE);

			hasWriteObject = writeObjectMethod != null;
			hasReadObject = readObjectMethod != null;

			writeReplaceMethod = getMethod("writeReplace", new Class[0], Object.class);

			readResolveMethod = getMethod("readResolve", new Class[0], Object.class);
			...
			/*
			 * getDeclaredFields does not specify or guarantee a specific order. Therefore,
			 * we sort the fields alphabetically, as does the IOGenerator.
			 */
			...
			/*
			 * Create the datastructures to cache the fields we need. Since we don't know
			 * the size yet, we create large enough arrays, which will later be replaced;
			 */
			...
			if (serial_persistent_fields == null) {
				/*
				 * Now count and store all the difference field types (only the ones that we
				 * should write!). Note that we store them into the array sorted by name !
				 */
				for (int i = 0; i < fields.length; i++) {
					...
					if ((modifiers & (Modifier.TRANSIENT | Modifier.STATIC)) == 0) {
						...
						try {
							...
							if (logger.isInfoEnabled()) {
								logger.info("Cannot set field " + field.getName() + " to accessible for class "
										+ clazz.getName() + ": " + e);
							}
						}
			...
			int size = double_count + long_count + float_count + int_count + short_count + char_count + byte_count
					+ boolean_count + reference_count;
			int index = 0;
	...
	/**
	 * Looks for a declaration of serialPersistentFields, and, if present, makes it
	 * accessible, and stores it in <code>serial_persistent_fields</code>.
	 */
	private void getSerialPersistentFields() {
		...
		if ((field.getModifiers() & mask) == mask) {
			...
			try {
				field.setAccessible(true);
				serial_persistent_fields = (java.io.ObjectStreamField[]) field.get(null);
			} catch (Exception e) {
				if (logger.isInfoEnabled()) {
					logger.info("Cannot set field " + field.getName() + " to accessible for class " + clazz.getName()
							+ ": " + e);
				}
			}
		}
	...
	/**
	 * Gets the field with fieldname indicated by the name in <code>of</code>. If
	 * not present, returns <code>null</code>.
	 */
	 ...
	 * <code>tp</code> in either <code>SerialPersistentFields</code>, if it exists,
	 * or in the <code>serializable_fields</code> array. An exception is thrown when
	 * such a field is not found.
	 *
