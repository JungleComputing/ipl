 ...
 * <p>
 * First, an {@link ibis.ipl.Ibis Ibis} instance must be created. This is achieved by
 * calling one of the <code>createIbis</code> methods of the {@link ibis.ipl.IbisFactory IbisFactory},
 * specifying the {@link ibis.ipl.IbisCapabilities capabilities} and port types required.
 * <p>
 ...
 * {@link ibis.ipl.SendPort#close() close} method. When this method is called, all connections the SendPort has
 * are disconnected. When a ReceivePort is no longer used it must be closed using the
 * {@link ibis.ipl.ReceivePort#close() close} method.
 * This call will block until connections to SendPorts are disconnected (by the SendPorts).
 * <p>
 * A {@link ibis.ipl.WriteMessage message} can be sent from a SendPort to the set of ReceivePorts it is connected to.
