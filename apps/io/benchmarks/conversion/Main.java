...
public final class Main {
    ...
    private void booleanConversion(Conversion conversion) {
        ...
        for (int bufferLength = MIN_BUF_SIZE; bufferLength <= MAX_BUF_SIZE; bufferLength *= 2) {
            ...
            boolean[] booleans = new boolean[bufferLength
                    / Conversion.BOOLEAN_SIZE];
            int ITERS = TOTAL_TEST_DATA_SIZE
                    / (booleans.length * Conversion.BOOLEAN_SIZE);
            byte[] buf = new byte[booleans.length * Conversion.BOOLEAN_SIZE];
            double thrp;
            double MBs = ITERS * booleans.length
                    * (double) Conversion.BOOLEAN_SIZE / (1 << 20);

            for (int warmup = 0; warmup < REPEATS; warmup++) {
                timer.reset();
                timer.start();
                for (int i = 0; i < ITERS; i++) {
                    conversion.boolean2byte(booleans, 0, booleans.length, buf,
                            0);
                }
                timer.stop();
            }
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("boolean2byte[" + booleans.length + "] ("
                    + ITERS + ") takes "
            ...
            for (int warmup = 0; warmup < REPEATS; warmup++) {
                ...
                timer.start();
                for (int i = 0; i < ITERS; i++) {
                    conversion.byte2boolean(buf, 0, booleans, 0,
                            booleans.length);
                }
                timer.stop();
            }
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("byte2boolean[" + booleans.length + "] ("
                    + ITERS + ") takes "
                    + doubleFmt((timer.totalTimeVal() / ITERS), 1)
                    + " us; thrp " + doubleFmt(thrp, 1) + " MB/s");
    ...
    private void shortConversion(Conversion conversion) {
        ...
        for (int bufferLength = MIN_BUF_SIZE; bufferLength <= MAX_BUF_SIZE; bufferLength *= 2) {
            ...
            int ITERS = TOTAL_TEST_DATA_SIZE
                    / (shorts.length * Conversion.SHORT_SIZE);
            byte[] buf = new byte[shorts.length * Conversion.SHORT_SIZE];
            double thrp;
            double MBs = ITERS * shorts.length * (double) Conversion.SHORT_SIZE
                    / (1 << 20);
            ...
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("short2byte[" + shorts.length + "] (" + ITERS
                    + ") takes " + doubleFmt((timer.totalTimeVal() / ITERS), 1)
                    + " us; thrp " + doubleFmt(thrp, 1) + " MB/s");
            ...
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("byte2short[" + shorts.length + "] (" + ITERS
                    + ") takes " + doubleFmt((timer.totalTimeVal() / ITERS), 1)
                    + " us; thrp " + doubleFmt(thrp, 1) + " MB/s");
    ...
    private void floatConversion(Conversion conversion) {
        ...
        for (int bufferLength = MIN_BUF_SIZE; bufferLength <= MAX_BUF_SIZE; bufferLength *= 2) {
            ...
            int ITERS = TOTAL_TEST_DATA_SIZE
                    / (floats.length * Conversion.FLOAT_SIZE);
            byte[] buf = new byte[floats.length * Conversion.FLOAT_SIZE];
            double thrp;
            double MBs = ITERS * floats.length * (double) Conversion.FLOAT_SIZE
                    / (1 << 20);
            ...
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("float2byte[" + floats.length + "] (" + ITERS
                    + ") takes " + doubleFmt((timer.totalTimeVal() / ITERS), 1)
                    + " us; thrp " + doubleFmt(thrp, 1) + " MB/s");
            ...
            thrp = MBs / (timer.totalTimeVal() / 1048576.0);
            System.out.println("byte2float[" + floats.length + "] (" + ITERS
                    + ") takes " + doubleFmt((timer.totalTimeVal() / ITERS), 1)
                    + " us; thrp " + doubleFmt(thrp, 1) + " MB/s");
