...
final class Main {
    ...
    private static long one_way(myServer s, int count, Object data)
            throws Exception {
        ...
        if (verbose)
            System.out.println("One way test took " + (end - start)
                    + " milliseconds");

        return (end - start);
    }

    private static long two_way(myServer s, int count, Object data)
            throws Exception {
        ...
        if (verbose)
            System.out.println("Two way test took " + (end - start)
                    + " milliseconds");
        return (end - start);
    }

    private static long runTest(myServer s, int count, int retries,
            Object data, boolean one_way) throws Exception {
        ...
        try {
            ...
            if (info.rank() == 0) {
                ...
                if (array) {
                    ...
                    if (verbose) {
                        System.out
                                .println("starting double[" + alen + "] test");
                    }

                    time = runTest(s, count, retries, data, one_way);
                    System.out.println("double[" + alen + "] = "
                            + tp(arraysize * count, time) + " MBytes/sec.");
                }
                ...
                if (list) {
                    ...
                    System.out.println("list(" + len + ") = "
                            + tp(len * count * List.PAYLOAD, time)
                            + " MBytes/sec.");
                }
                ...
                if (dlist) {
                    ...
                    time = runTest(s, count, retries, data, one_way);
                    System.out.println("dlist(" + len + ") = "
                            + tp(len * count * DList.PAYLOAD, time)
                            + " MBytes/sec.");
                }
                ...
                if (tree) {
                    ...
                    time = runTest(s, count, retries, data, one_way);
                    System.out.println("tree(" + len + ") = "
                            + tp(len * count * Tree.PAYLOAD, time)
                            + " MBytes/sec.");
                }
                ...
                if (oarray) {
                    ...
                    if (verbose) {
                        System.out.println("starting object array[" + len
                                + "] test");
                    }

                    time = runTest(s, count, retries, temp, one_way);
                    System.out.println("object[" + len + "] = "
                            + tp(len * count * Data.PAYLOAD, time)
                            + " MBytes/sec.");
                }
