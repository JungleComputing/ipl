...
class SOR extends UnicastRemoteObject implements SORInterface {
    ...
    SORInterface[] table;                      /* a table of all SOR threads */
    ...
    public void setTable( SORInterface[] table ) {
        this.table = table;
    }
    ...
    {
        for( int r = lb+1 ; r <= ub-1 ; r++ ) {
            int c = 1 + ( ( r+color ) & 1 );

            while ( c < ncol-1 ) {
                double gNew = stencil( r,c );
                double diff = abs( gNew - g[r][c] );
