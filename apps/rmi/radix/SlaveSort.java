public class SlaveSort {
    ...
    SlaveSort(String myName, String masterName, int cpu) throws Exception {
        ...
        myCpu = cpu;
        master = (RadixMasterInterface) RMI_init.lookup("//" + masterName
                + "/RadixMaster");
    ...
    public void merge_Ranks(int[] key_Density, int[] rank_Me_Mynum,
            int[] rank_Ff_Mynum, int m) throws Exception {
        //shared tree
        Prefix_Node myNode;
    ...
    public void tree_Reduce_Sum(int[] key_Density, int[] rank_Me_Mynum)
            throws Exception {
        //shared tree
    ...
    synchronized void t_Node_Put_Set(int place, int[] key_Density,
            int[] rank_Me_Mynum) throws Exception {
        tree.putSet(place, key_Density, rank_Me_Mynum);
    }

    synchronized void t_Node_Put_Dens(int place, int[] key_Density)
            throws Exception {
    ...
    public void move_Keys(int[] rank_Me_Mynum, int[] rank_Ff_Mynum, int shift)
            throws Exception {
    ...
    public void check() {
        ...
        for (int j = 0; j < amount - 1; j++) {
            if (sh_My_Keys.part[j] > sh_My_Keys.part[j + 1]) {
                System.out.println("keys not sorted, amount = " + amount
                        + ", j = " + j);
                System.exit(1);
            }
        }
