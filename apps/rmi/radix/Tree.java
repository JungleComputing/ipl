...
public class Tree extends UnicastRemoteObject implements TreeInterface {
    ...
    public synchronized void putDensity(int node, int[] density)
            throws RemoteException {
        tree[node].densities = density;
    }

    public synchronized void putRank(int node, int[] rank)
            throws RemoteException {
        tree[node].ranks = rank;
    }

    public synchronized void putSet(int node, int[] density, int[] rank)
            throws RemoteException {
        tree[node].densities = density;
        tree[node].ranks = rank;
    }
