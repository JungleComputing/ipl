...
public class RadixMaster extends UnicastRemoteObject implements RadixMasterInterface, Runnable{
	...
	System.out.println("#cpu  totaltime\t histogramtime\t sorttime\t mergetime\t permutetime");
	...
	System.out.println();
	System.out.println("Min\t" + mint + "\t  " + minHistogram + "\t\t  " + minSort + "\t\t  " + minMerge + "\t\t  " + minPermute);
	...
	else while (num_trees < number_Of_Processors) {
	    try {
		wait();
	    } catch (Exception e) {
    ...
    public synchronized PartsInterface[] getParts(PartsInterface part, int cpunum) throws RemoteException {
	num_parts++;
	iparts[cpunum] = part;
	if (num_parts == number_Of_Processors) {
	    notifyAll();
	}
	else while (num_parts < number_Of_Processors) {
	    try {
		wait();
	    } catch (Exception e) {
		throw new RemoteException(e.toString());
