...
class Slave extends UnicastRemoteObject implements SlaveInterface {
    ...
    Slave(String masterName, int cpu, int cpus, int N, int M, int rootN,
            int rowsperproc, double[] u, double[] u2, int[][] dis, PoolInfo d,
            int rounds) throws Exception {

        master = (MasterInterface) RMI_init.lookup("//" + masterName
                + "/FFT_Master");
        ...
        for (int warmup = 0; warmup < rounds; warmup++) {
            ...
            if (myCpu == 0) {
                System.out.println("FFT round " + warmup + " time "
                        + (end - start) / 1000.0);
            }
        }
        ...
        if (myCpu == 0) {
            ...
            System.out.println("FFT, iterate " + rounds + "; M = " + M
                    + " time " + total / 1000.0 + "; per round " + per_round
                    / 1000.0);
            System.out.println("Application: FFT " + M + " -warmup "
                    + (rounds - 1) + "; time: " + total / 1000.0 + " seconds");
        }
    }
    ...
    void pageOut(double[] x, int[][] ind) {
        try {
            ...
            System.out.println("Couldn't get/set matrix values from master "
                    + e);
            System.exit(1);
        }
    }
    ...
    void doFFT(double direction, double[] u, double[] u2, int myFirst,
            int myLast) throws Exception {
