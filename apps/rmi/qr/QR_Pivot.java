...
class QR_Pivot {
	...
	QR_Pivot(PoolInfo info, int N, i_Data data, i_Reduce reduce, BroadcastObject broadcast) {
	...
	void factor_my_columns(int pivot, int first_col, int [] local_perm, int iter, double [] v) {
...
//		RuntimeSystem.barrier();
		...
		for (int iter = 0; iter < min(m, n); iter++) {
			...
			if (cpu == 0 && (iter % 10 == 0)) { 
				System.out.println("Iter " + iter + " time so far = " + (System.currentTimeMillis()-time));		
			} 
			...
			if (iter > 0) {
				...
				for (int i=first_col;i<my_cols;i++) {
					...
					*/			
					...
					if (subcol_norm2[local_perm[i]] < 400 * FLT_EPSILON * safe_subcol_norm2[local_perm[i]]) {
						// System.out.println("Bingo col " + local_perm[i]+ " iter " +iter);
						subcol_norm2[local_perm[i]]      = eucl_norm2(local_perm[i], iter);
			...
			if (pivot_tolerance * pivot.max_over_max_cols < pivot.norm) {
				if (cpu == 0) {
					System.err.println("Need load balancing phase iter "+ iter +" norm "+ pivot.norm + " max(cols)norm "+pivot.max_over_max_cols);
				}
			}
			...
			*/
			...
			if (pivot_owner == cpu) {			       
				...
				if (ajnorm == 0.0) {
					System.err.println("Singular matrix : column "+ iter+ "norm = 0");
				}
