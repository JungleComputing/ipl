...
public class ForcesEnergy implements ConstInterface {
	...
	for (int mol = 0; mol < nrmols;  mol++) {
		...
		if (WaterMaster.USE_VM) {
		    var.vm[dir][mol] = C1 * var.f[DISP][mol][dir][O]
			+ C2 * (var.f[DISP][mol][dir][H1]
		...
		System.out.println("r1 = " + (float)r1 + ", r2 = " + (float)r2);
		System.out.println("cos = " + (float)cos + ", acos(cos) = " + (float)Math.acos(cos) + ", sin = " + (float)sin);
		System.out.println("acos(cos) - ANGLE = " + ((float)Math.acos(cos) - ANGLE) + ", (Math.acos(cos) - ANGLE) * ROH = " + (float)(Math.acos(cos) - ANGLE) * ROH);
		System.out.println("dt = " + (float)dt + ", dts = " + (float)dts);
		System.out.println("dr1 = " + (float)dr1 + ", dr1s = " + (float)dr1s);
		System.out.println("dr2 = " + (float)dr2 + ", dr2s = " + (float)dr2s);
	    }
	    ...
	    if (WaterMaster.VERBOSE) {
		System.out.println("r1s = " + (float)r1s + ", r2s = " + (float)r2s);
	    }
		...
		if (WaterMaster.VERBOSE) {
		    System.out.println(dir + ": dr11 = " + (float)dr11[dir] + ", dr23 = " + (float)dr23[dir] + ", dt1 = " + (float)dt1[dir] + ", dt3 = " + (float)dt3[dir]);
		}
		...
		* ROHI2;
		...
		if (WaterMaster.VERBOSE) {
		    System.out.println("t1 = " + (float)t1 + ", t2 = " + (float)t2);
		}
	...
	for (mol = 0; mol < nrmols; mol++) {
	    ...
	    if (nmol % 2 == 0 && ((mol % 2 == 0 && (mol < halfMol)) || (mol % 2 == 1 && mol > halfMol))) {
		compLast -= 1;
	    }
	    //System.out.print(".");
	    //System.out.flush();
		...
		if(cpu == -1)
		    ...
		    computeDistances(var.f[DISP][mol], allPos[cpu][offset], xl, boxh, boxl);

		kc=0;

		for (k = 0; k < 9; k++) {
		    rs[k] = xl[0][k] * xl[0][k] + xl[1][k] * xl[1][k] + xl[2][k] * xl[2][k];
		    if (rs[k] > cut2)
			kc++;
		}
		if (kc != 9) {
		    ...
		    for (k = 1; k < 5; k++) {
			if (rs[k] < cut2) {
			    ff[k]= -QQ2 / (rs[k] * Math.sqrt(rs[k])) - (2 * ref1);
			    lvir = lvir + ff[k] * rs[k];
			}
			if (rs[k+4] <= cut2) {
		    ...
		    if (kc == 0) {
			rs[9] = xl[0][9] * xl[0][9] + xl[1][9] * xl[1][9] + xl[2][9] * xl[2][9];
			rl[9] = Math.sqrt(rs[9]);
			...
			for (k = 10; k < 14; k++) {
			    ...
			    rs[k] = xl[0][k] * xl[0][k] + xl[1][k] * xl[1][k] + xl[2][k] * xl[2][k];
			    rl[k] = Math.sqrt(rs[k]);
			    ff[k] = (AB3 * Math.exp(-B3 * rl[k]) - AB4 * Math.exp(-B4 * rl[k]))/rl[k];
			    lvir = lvir + ff[k] * rs[k];
			}
		    }
		    if(cpu == -1)
			updateForces(var.f[dest][mol], var.f[dest][offset], xl, ff);
		    else
			updateForces(var.f[dest][mol], allVal[cpu][offset], xl,ff);
		}
		// System.out.println("offset = " + offset);
	...
	for(int dir = XDIR; dir <= ZDIR; dir++){
	    xma = C1 * myAtoms[dir][O] + C2 * (myAtoms[dir][H1] + myAtoms[dir][H2]);
	    xmb = C1 * farAtoms[dir][O] + C2 * (farAtoms[dir][H1] + farAtoms[dir][H2]);
		...
		if (Math.abs(xl[dir][i]) > boxh) {
		    boxl = (xl[dir][i] < 0 ) ? ( (boxl < 0) ? boxl : -boxl) : ( (boxl < 0) ? -boxl : boxl);
    ...
    public void updateForces(double[][] myAtoms, double[][] farAtoms, double[][] xl, double ff[]){
	...
	for(int i = XDIR; i <= ZDIR; i++){
		...
		gg[k+1] = ff[k] * xl[i][k];
		if (WaterMaster.VERBOSE) {
		    System.out.println("gg[" + k+1 + "] = " + (float) gg[k+1]);
		}
	    }
	    g110 = gg[10] + gg[1] * C1;
	    g23 = gg[2] + gg[3];
	...
	for (int mol = 0; mol < nrmols; mol++) {
	    if (WaterMaster.USE_VM) {
		var.vm[XDIR][mol] = C1 * var.f[DISP][mol][XDIR][ O]
		    + C2 * (var.f[DISP][mol][XDIR][H1]
			    + var.f[DISP][mol][XDIR][H2]);
		var.vm[YDIR][mol] = C1 * var.f[DISP][mol][YDIR][O]
		    + C2 * (var.f[DISP][mol][YDIR][H1]
			    + var.f[DISP][mol][YDIR][H2]);
		var.vm[ZDIR][mol] = C1 * var.f[DISP][mol][ZDIR][O]
		    + C2 * (var.f[DISP][mol][ZDIR][H1]
			    + var.f[DISP][mol][ZDIR][H2] );
	    }
		...
		((var.f[DISP][mol][ZDIR][O] - var.f[DISP][mol][ZDIR][H1]) *
		 (var.f[DISP][mol][ZDIR][O] - var.f[DISP][mol][ZDIR][H2]));
	...
	for (int mol = 0; mol < nrmols; mol++) {
	    ...
	    if (nmol % 2 == 0 && ((mol % 2 == 0 && (mol < half_mol)) || ((mol%2 == 1) && mol > half_mol)))
		compLast -= 1;
		...
		if (kc != 9) {
		    ...
		    lpotr += - QQ2/rl[1] - QQ2/rl[2] - QQ2/rl[3] - QQ2/rl[4]
			...
			lpotr += A1 * Math.exp(-B1 * rl[9])
			    + A2 * (Math.exp(-B2 * rl[5]) + Math.exp(-B2 * rl[6])
				    + Math.exp(-B2 * rl[7]) + Math.exp(-B2 * rl[8]))
			    + A3 * (Math.exp(-B3 * rl[10]) + Math.exp(-B3 * rl[11])
				    + Math.exp(-B3 * rl[12]) + Math.exp(-B3 * rl[13]))
			    - A4 * (Math.exp(-B4 * rl[10]) + Math.exp(-B4 * rl[11])
				    + Math.exp(-B4 * rl[12]) + Math.exp(-B4 * rl[13]));
		    }
		}
		if(offset == (ub-1)){
		    //step to next processor
