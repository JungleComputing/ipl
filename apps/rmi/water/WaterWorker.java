...
public class WaterWorker extends UnicastRemoteObject implements WaterWorkerInterface, ConstInterface {
	...
	for (int ordr = 0; ordr < MAXODR; ordr++) {
		...
		for (int k = 0; k < var.f[ordr][j].length; k++) {
		    ...
		    System.out.println("            " + (float) var.f[ordr][j][k][0] + ",  " + (float) var.f[ordr][j][k][1] + ", " + (float) var.f[ordr][j][k][2]);
		}
	    }
	}
    }
    ...
    public void doPoteng(double ttmv, double tvir, int iteration) throws Exception {
	double xtt = 0.0;
	double lpotA = 0.00;
	double lpotR = 0.00;
	double lpotRF = 0.00;
