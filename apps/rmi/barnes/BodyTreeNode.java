...
strictfp class BodyTreeNode {
    ...
    protected void Initialize(GlobalData g, double center_x, double center_y,
        double center_z, double HalfSize, int depth) {
    ...
    public BodyTreeNode(GlobalData g, double center_x, double center_y,
        double center_z, double HalfSize, int depth) {
    ...
    int dumpTree(int level, int maxlevel) {
        ...
        if (level <= maxlevel) {
            ...
            String str = "l" + level + ", center (" + btnCenter_x + ","
                + btnCenter_y + "," + btnCenter_z + "), size " + btnHalfSize
                + ", cofm (" + btnCenterOfMass_x + "," + btnCenterOfMass_y
                + "," + btnCenterOfMass_z + "), leaf " + (btnIsLeaf ? 1 : 0)
                + ", bodies " + bodies + ": ";
    ...
    private void createChild(int i) {
        ...
        case 7:
            btnChildren[7] = new BodyTreeNode(btnGd, btnCenter_x + newSize,
                btnCenter_y + newSize, btnCenter_z + newSize, newSize,
                btnDepth + 1);
            break;
        case 6:
    ...
    private BodyTreeNode findContainingChild(int bodyIndex) {
        ...
        if (Math.abs(dx) > btnHalfSize || Math.abs(dy) > btnHalfSize
            || Math.abs(dz) > btnHalfSize) {

            debugFatalStr("Error! Tree inconsistency detected, while trying to distribute body: "
                + bodyIndex);
            debugFatalStr("Cell Center: [ " + btnCenter_x + ", " + btnCenter_y
                + ", " + btnCenter_z + " ]");
            ...
            if (btnChildren[index] == null) createChild(index);
    ...
    private BodyTreeNode findContainingChild(CenterOfMass c) {
        ...
        if (Math.abs(dx) > btnHalfSize || Math.abs(dy) > btnHalfSize
            ...
            debugFatalStr("Cell Center: [ " + btnCenter_x + ", " + btnCenter_y
                + ", " + btnCenter_z + " ]");
            debugFatalStr("Cell Size:   " + btnHalfSize + " dx=" + dx + " dy="
                + dy + " dz=" + dz);
    ...
    int ComputeAccelerationsBarnes(BodyTreeNode root) {
        ...
        if (btnIsLeaf) {
            ...
            for (i = 0; i < 8; i++) {
                if (btnChildren[i] != null)
                    interactions += btnChildren[i]
                        .ComputeAccelerationsBarnes(root);
            }
        }
        return interactions;
    }
