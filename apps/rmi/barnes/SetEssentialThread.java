...
strictfp public class SetEssentialThread extends Thread {
    ...
    public synchronized void put(int Source, int bCount, double[] bp,
        int cCount, double[] cp) {
        while (filled) {
            try {
                wait();
            } catch (Exception e) {
    ...
    public synchronized void put(int Source, int bCount, SendBody[] b,
        int cCount, CenterOfMass[] c) {
        while (filled) {
            waiters++;
            try {
                wait();
