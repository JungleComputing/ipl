...
class ACP {
	...
	public ACP(int numVariables, int numValues, int numConnections, int numRelations, int numRelationPairs, int seed, 
		   i_Data data, i_Work work, Matrix matrix, i_Matrix group) { 
	...
	public static void main(String args[]) {		
		...
		try {
			...
			if (cpu == 0) { 
				...
				Work  w = new Work(numVariables, cpus, numVariables, numValues, numConnections, numRelations, numRelationPairs, seed);
				Group.join("Work", w);
			} 
			...
			m = Group.findMethod(data, "void put(int, int, long, long, int, int, int)");
