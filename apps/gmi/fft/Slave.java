...
class Slave extends GroupMember implements i_Slave {
    ...
    Slave(int cpus, int N, int M, int rootN, int rowsperproc, double[] u,
            double[] u2, int[][] dis) {
    ...
    public void setGroup(i_Slave group, int cpu) {
        ...
        try {
            setMethod = Group.findMethod(slaveGroup,
    ...
    void pageOut(double[] x, int[][] ind) {
        try {
            ...
            for (int i = 0; i < rootN; i++) {
                ...
                if (i % rowsperproc == (rowsperproc - 1)) {
                    ...
                    if (slaveNr == myCpu) {
                        ...
                        setMethod.configure(new SingleInvocation(slaveNr),
                                new ReturnReply(slaveNr));
            ...
            System.out.println(myCpu
                    + "PageOut: Couldn't get/set matrix values from master "
                    + e);
            e.printStackTrace();
            System.exit(1);
        }
        ...
        try {
            ...
            for (int i = 0; i < cpus; i++) {
                ...
                if (myCpu == slaveNr) {
                    ...
                    getMethod.configure(new SingleInvocation(slaveNr),
                            new ReturnReply(slaveNr));
            ...
            System.out
                    .println(myCpu
                            + "PageIn: Couldn't get/set matrix values from master "
                            + e);
            System.exit(1);
        }
    }
    ...
    void run() throws Exception {
        ...
        if (Group.rank() == 0) {
            System.out.println("FFT, M = " + M + " time "
                    + ((end - start) / 1000.0));
            if (failed) {
                System.exit(1);
            }
