...
class Main {
	...
	public static void main(String[] argv) {
		...
		try {
			...
			CombinedInvocation clat = new CombinedInvocation("Latency", rank, nodes, new LatCombiner(), new GroupInvocation());
			m.configure(clat, new DiscardReply());
			
			m = Group.findMethod(group, "void storeT(byte[])");
			CombinedInvocation ctp = new CombinedInvocation("Throughput", rank, nodes, new TPCombiner(size), new GroupInvocation());
