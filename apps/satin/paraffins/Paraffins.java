...
final class RecursivePartition implements IntArrayEnumeration {
    ...
    RecursivePartition(int sum_of_elements, int number_of_elements,
        ...
        result = new Vector(number_of_elements * (max_element - min_element)
                / 2);
        position = 0;
    ...
    void recursively_partition(int level, int remain, int prev_element, int[] p) {
        ...
        if (remaining_levels > 0) {
            int smallest = Math.max(prev_element, remain - max_element
                    * remaining_levels);
            int largest = Math.min(remain / (remaining_levels + 1), remain
                    - min_element * remaining_levels);
...
final class Paraffins implements Constants {
    ...
    void work() {
        ...
        for (int i = 0; i < size + 1; i++) {
            System.out
                    .println("size " + i + " has " + counts[i] + " molecules");
        }
    }
    ...
    void gen_ccps_of_size(int size) {
        ...
        // on these partitions
        IntArrayEnumeration e = new RecursivePartition(size - 1, 4, 0,
                (size - 1) / 2);
        int[] partition;
        while ((partition = e.nextElement()) != null) {
    ...
    void gen_bcps_of_size(int size) {
        ...
        // and then actually creates new CCP molecules  based
        // on these partitions
        IntArrayEnumeration e = new RecursivePartition(size, 2, size / 2,
                size / 2);
        int[] partition;
        while ((partition = e.nextElement()) != null) {
    ...
    void enum_rad_tuples(int[] p, int kind, int size) {
        ...
        recursively_enumerate(p, kind, size, radical_tuple, 0, radicals[p[0]],
                0, p[0]);
    }
    ...
    void recursively_enumerate(int[] p, int kind, int size,
            Radical[] radical_tuple, int level, Vector remainder,
            int remainderpos, int prev_element) {
    ...
    private static void usage() {
        System.out.println("usage: java Main " + "<number of carbon atoms>");
    }
