...
public class SuffixArray implements Configuration, Magic, java.io.Serializable {
    ...
    private int disjunctMatch(int ix0, int ix1) {
        ...
        while ((ix0 + n) < ix1 && (text[ix0 + n] == text[ix1 + n])
                && (text[ix0 + n] != STOP)) {
            n++;
        }
        return n;
    ...
    private boolean rightIndexOrder1(int i0, int i1) {
        ...
        System.out.println("i0=" + i0 + " i1=" + i1 + " text[" + i0 + "]="
                + text[i0] + " text[" + i1 + "]=" + text[i1] + " res=" + res);
        return res;
    }
    ...
    private void sort(int indices[], int start, int end, int offset) {
                ...
                for (int j = 0; j < (length - jump); j++) {
                    ...
                    if (!rightIndexOrder(ixi + offset, ixj + offset)) {
                        ...
                        if (!rightIndexOrder(ixj + offset, ixi + offset)) {
                            System.out.println("There is no right order for "
                                    + ixj + " and " + ixi);
                        }
                        done = false;
                    }
        ...
        if (false) {
            for (int i = start; i < end; i++) {
                System.out.println("i=" + i + " indices[" + i + "]="
                        + indices[i] + " offset=" + offset + " text["
                        + (indices[i] + offset) + "]="
                        + text[indices[i] + offset]);
            }
        }
    }
     ...
     */
    private int sort1(int next[], int indices1[], boolean comm[], int p,
            int indices[], int start, int end, int offset) {
        if (end - start < 1) {
    ...
    private void newsort(int indices[], int commonality[], int start, int end) {
        ...
        for (int i = start + 1; i < end; i++) {
            commonality[i] = 1 + commonLength(indices[i - 1] + 1,
                    indices[i] + 1);
        }
    }
    ...
    public void applyCompression(Step s) {
        ...
        if (doVerification) {
            ...
            if (length + gain != oldLength) {
                System.out.println("Error: predicted gain was " + gain
                        + ", but realized gain is " + (oldLength - length));
            }
        }
    }
