...
public final class Breeder extends ibis.satin.SatinObject implements
        BreederInterface {
    ...
    // extrapolated clone.
    private static Genes extrapolateGenes(float scale, Genes g, Genes oldG,
            Genes max, Genes min) {
    ...
    private static Genes mutateGenes(Random rng, Genes g, float step,
            Genes max, Genes min) {
        Genes res = (Genes) g.clone();
     ...
     */
    private Result breedNextGeneration(Random rng, SATProblem pl[],
            Genes genes, int bestD) {
        float step = 0.15f;
        ...
        if (prevBestGenes != null) {
            ...
            g[slot] = extrapolateGenes(0.3f, genes, prevBestGenes, maxGenes,
                    minGenes);
