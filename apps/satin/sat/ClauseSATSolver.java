...
public class ClauseSATSolver {
    ...
    static void propagateTrueValue(Context ctx, final int var) {
        ...
        for (int ix = 0; ix < poscls.length; ix++) {
            ...
            satisfied[cno] = true;
            if (traceSolver) {
                System.out.println("Assignment var[" + var
                        + "]=true satisfies clause ("
                        + ctx.problem.getClauseLabel(cno) + ")");
            }
        }
        int terms[] = ctx.terms;
     ...
     */
    static int[] buildNewTerms(final int old_terms[], final Clause clauses[],
            final int clauseCount, final byte assignments[]) {
    ...
    static void verifySolution(Context ctx) {
        ...
        if (unsat >= 0) {
            System.err.println("Verification failed: clause ("
                    + ctx.problem.getClauseLabel(unsat) + ") is not satisfied");
        }
    }
            ...
            if (gal[ix] != -1) {
                ...
                if (ctx.problem.isSatisfied(gal)) {
                    if (traceSolver || printSatSolutions) {
                        System.out
                                .println("Found a valid generalized solution by omitting term "
                                        + (olda == 0 ? "!" : "") + ix);
                    }
                    addSolutionList(ctx);
                }
    ...
    static void addSolutionList(Context ctx) {
        ...
        if (ctx.solutioncount == oldcount) {
            ...
            if (traceSolver) {
                System.out
                        .println("Avoiding duplicate solutions with new clause ("
                                + lbl + ")");
            }
        }
    }
    ...
    static boolean tryAssignments(Context ctx) {
        ...
        if (traceSolver) {
            System.err.println("(" + ctx.level + ") branching on " + min_terms
                    + "-unsatisfied clause (" + cl.label + ")");
        }
        ...
        for (int i = 0; i < cl.pos.length; i++) {
            ...
            if (ctx.assignments[var] == -1) {
                ...
                if (traceSolver) {
                    System.err.println("(" + ctx.level
                            + ") trying assignment var[" + var + "]=true");
                }
                {
                    ...
                    System.arraycopy(old_satisfied, 0, ctx.satisfied, 0,
                            old_satisfied.length);

                    // Create a new `terms' array for this recursion.
                    int old_terms[] = ctx.terms;
                    ctx.terms = buildNewTerms(old_terms, ctx.problem.clauses,
                            ctx.problem.getClauseCount(), ctx.assignments);
                ...
                if (traceSolver) {
                    System.err.println("(" + ctx.level
                            + ") retracting assignment var[" + var + "]=true");
                }
        ...
        for (int i = 0; i < cl.neg.length; i++) {
            ...
            if (ctx.assignments[var] == -1) {
                ...
                if (traceSolver) {
                    System.err.println("(" + ctx.level
                            + ") trying assignment var[" + var + "]=false");
                }
                ...
                if (traceSolver) {
                    System.err.println("(" + ctx.level
                            + ") retracting assignment var[" + var + "]=false");
                }
    ...
    public static void main(String args[]) throws java.io.IOException {
        ...
        System.out.println("application time " + "ClauseSATSolver (" + args[0]
                + ") took " + time + " seconds");
        if (res.length == 1) {
