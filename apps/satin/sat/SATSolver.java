...
public final class SATSolver extends ibis.satin.SatinObject implements
        SATInterface, java.io.Serializable {
     ...
     */
    public void leafSolve(int level, SATContext ctx, int var, boolean val,
            boolean learnTuple) throws SATResultException, SATJumpException {
     ...
     * @param learnTuple Propagate any learned clauses as active tuple?
     */
    public void solve(int level, int localLevel, SATContext ctx, int var,
            boolean val, boolean learnTuple) throws SATException {
        if (!localJob()) {
        ...
        if (localLevel < 10) {
            try {
                ...
                solve(level + 1, localLevel + 1, firstctx, nextvar, firstvar,
                        learnTuple);
                SATContext secondctx = (SATContext) ctx.clone();
                solve(level + 1, localLevel + 1, secondctx, nextvar, !firstvar,
                        learnTuple);
                sync();
            } catch (SATJumpException x) {
                if (x.level < level) {
                    if (traceJumps) {
                        System.err.println("JumpException passes level "
                                + level + " heading for level " + x.level);
                    }
                    throw x;
                }
            }
    ...
    static SATSolution solveSystem(final SATProblem p, boolean learnTuple) {
        ...
        System.err.println("Learned " + (newClauseCount - oldClauseCount)
                + " clauses.");
        return res;
    }
    ...
    public static void main(String args[]) throws java.io.IOException {
        if (args.length != 1) {
            System.err
                    .println("Exactly one filename argument required, but I have "
                            + args.length + ":");
        ...
        System.out.println("application time " + "SATSolver (" + args[0]
                + ") took " + time + " seconds");
        if (res == null) {
