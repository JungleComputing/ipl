class Polygon3D extends Plane3D
{
    ...
    Polygon3D(World world,
	    Surface  surf,
	    int pts,
	    Vector3D [] p)
    {
	super(world,
		Vector3D.CrossProduct(Vector3D.Substract(p[2], p[0]),
		    Vector3D.Substract(p[1], p[0])),
		p[0],
		surf);
	...
	if (dist > 0.000001 && dist < ray.min_dist)
	{
	    ...
	    Vector3D p = new Vector3D((dist * ray.vec.x) + ray.pos.x,
		    (dist * ray.vec.y) + ray.pos.y,
		    (dist * ray.vec.z) + ray.pos.z);
	    if (PointInPoly(p))
	    {
		ray.min_dist = dist;
		ray.closest  = this;
	...
	double angle = Math.abs(Vector3D.DotProduct(vec, normalized_normal) * surface.angle_dep);
	...
	GfxColor reflected_color = world.Reflect(normalized_normal,
		vec,
		pos,
		depth);
