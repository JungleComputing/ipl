...
class Sphere3D extends Object3D implements java.io.Serializable {
    ...
    Sphere3D(World world, Vector3D center, double rad, Surface c) {
        super(world, c);
    ...
    GfxColor GetColor(Vector3D pos, Vector3D vec, int depth) {
        ...
        double angle = Math.abs(Vector3D.DotProduct(normal, vec)
                * surface.angle_dep);
        ...
        GfxColor c = GfxColor.Interpolate(surface.color.Multiply((float) angle)
                .Multiply(light), world.background, surface.diffusity);

        GfxColor reflected_color = world.Reflect(normal, vec, pos, depth);

        GfxColor refracted_color = world.Refract(inv_normal, vec, pos, depth,
                surface.refraction);
        ...
        c.r = (c.r + (refracted_color.r * surface.refraction) + (reflected_color.r * surface.reflection))
                / surface.absorbtion;

        c.g = (c.g + (refracted_color.g * surface.refraction) + (reflected_color.g * surface.reflection))
                / surface.absorbtion;

        c.b = (c.b + (refracted_color.b * surface.refraction) + (reflected_color.b * surface.reflection))
                / surface.absorbtion;
