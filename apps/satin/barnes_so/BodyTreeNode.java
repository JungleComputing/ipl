...
final class BodyTreeNode implements java.io.Serializable {
    ...
    /* bodyCount is only used during (initial) tree building. After trimming,
     bodies.length must be used */
    int bodyCount;

    /* The part of space this node represents.
     The fields are used during tree construction,
     so they can not be made transient.
     ...
     */
    BodyTreeNode(BodyTreeNode original, BodyTreeNode job) {
	int i;
     ...
     */
    public double[] computeForcesFor(double pos_x, double pos_y, double pos_z) {
        ...
        if (children == null) {
            ...
            for (i = 0; i < 8; i++) {
                double[] childresult;
                if (children[i] != null) {
                    childresult = children[i].computeForcesFor(pos_x, pos_y, pos_z);
    ...
    /**
     * debug version of computeForcesFor(pos_x, pos_y, pos_z)
     */
    public double[] computeForcesForDbg(double pos_x, double pos_y, double pos_z,
            boolean debug) {
        double diff_x, diff_y, diff_z;
        double[] totalAcc = new double[3];
        double dist, distsq, factor;
        ...
        if (children == null) {
            ...
            for (i = 0; i < 8; i++) {
                double[] childresult;
                if (children[i] != null) {
                    childresult = children[i].computeForcesForDbg(pos_x, pos_y,
                            pos_z, debug);
                    totalAcc[0] += childresult[0];
                    totalAcc[1] += childresult[1];
                    totalAcc[2] += childresult[2];
     ...
     * calling interactTree.computeForcesFor(bodies[i].pos) for all the bodies
     * when 'this' is a leaf node.
     */
    public LinkedList computeForcesSequentially(BodyTreeNode interactTree) {
        LinkedList result;
        int i;
        ...
        if (children == null) { //leaf node
            ...
            if (BarnesHut.ASSERTS && (bodies == null || bodies.length == 0)) {
                System.err.println("computeForcesSequentially(interactTree): "
            ...
            for (i = 0; i < bodies.length; i++) {
                ...
                acc = interactTree.computeForcesFor(bodies[i].pos_x, bodies[i].pos_y,
                        bodies[i].pos_z);
            ...
            for (i = 0; i < 8; i++) {
                if (children[i] != null) {
                    res[i] = children[i].computeForcesSequentially(interactTree);
                    lastValidChild = i;
                }
            }

            result = BarnesHut.combineResults(res, lastValidChild);
        }
        return result;
    }
