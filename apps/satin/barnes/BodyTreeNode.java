...
final class BodyTreeNode extends ibis.satin.SatinObject implements
    ...
    private void addBody(Body b, int maxLeafBodies) {
        ...
        if (children != null) { // cell node
            ...
            if (BarnesHut.ASSERTS && outOfRange(b.pos)) {
                ...
                System.err.println("     Center: " + center + " halfSize: "
                        + halfSize);
        ...
        if (children == null) { //leaf node
            ...
            for (i = 0; i < 8; i++) {
                if (children[i] != null) {
                    ...
                    centerOfMass.x += children[i].centerOfMass.x
                            * children[i].totalMass;
                    centerOfMass.y += children[i].centerOfMass.y
                            * children[i].totalMass;
                    centerOfMass.z += children[i].centerOfMass.z
                            * children[i].totalMass;
            ...
            for (i = 0; i < 8; i++) {
                if (children[i] != null) {
                    if (threshold > 0) {
                        ...
                        BodyTreeNode necessaryTree = new BodyTreeNode(
                                interactTree, children[i]);
    ...
    public void print(java.io.PrintStream out, int level) {
        ...
        out.print("CoM at " + centerOfMass.x + ", " + centerOfMass.y + ", "
                + centerOfMass.z);
