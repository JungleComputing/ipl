...
class Decompress {
    ...
    public static ByteBuffer decompress(byte text[]) {
        ...
        while (pos < text.length) {
            ...
            if (c == Magic.BACKL1D1) {
                int len = Helpers.decodeByte(text[pos++])
                        + Configuration.MINIMAL_SPAN;
                int d = Helpers.decodeByte(text[pos++])
                        + Configuration.MINIMAL_SPAN;
                out.replicate(d, len);
            } else if (c == Magic.BACKL2D1) {
                int len = Helpers.decodeShort(text[pos++], text[pos++])
                        + Configuration.MINIMAL_SPAN;
                int d = Helpers.decodeByte(text[pos++])
                        + Configuration.MINIMAL_SPAN;
                out.replicate(d, len);
                ...
                int len = Helpers.decodeByte(text[pos++])
                        + Configuration.MINIMAL_SPAN;
                int d = Helpers.decodeShort(text[pos++], text[pos++])
                        + Configuration.MINIMAL_SPAN;
                out.replicate(d, len);
