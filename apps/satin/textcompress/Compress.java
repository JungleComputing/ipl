...
class Compress extends ibis.satin.SatinObject implements CompressorInterface {
    ...
    private static boolean matchesMinSpan(byte text[], int p1, int p2) {
        return text[p1] == text[p2] && text[p1 + 1] == text[p2 + 1]
                && text[p1 + 2] == text[p2 + 2] && text[p1 + 3] == text[p2 + 3];
    }
     ...
     */
    public Backref shallowEvaluateBackref(final byte text[], int backpos,
            int pos) {
        Backref r;
     ...
     * job.
     */
    public Backref shallowEvaluateBackrefJob(final byte text[], int backpos,
            int pos) {
     ...
     */
    public Backref selectBestMove(byte text[], int backrefs[], int pos,
            int bestpos, int max_shortening, int depth, int max_depth) {
        Backref mv = null;
        ...
        if (traceLookahead) {
            ...
            System.out.println("D" + depth + ": @" + pos
                    + ": selecting best move improving on @" + bestpos);
        }
        ...
        if (!haveAlternatives) {
            ...
            if (mv != null && depth > 0 && depth < max_depth) {
                ...
                Backref mv1 = selectBestMove(text, backrefs, pos + 1, pos + 1,
                        max_shortening, depth + 1, max_depth);
                mv.addGain(mv1);
            }
            if (traceLookahead) {
                generateIndent(System.out, depth);
                System.out.println("D" + depth
                        + ": no backrefs, so only move is: " + mv);
            }
            return mv;
        }
        ...
        if (traceLookahead) {
            ...
            if (results != null) {
                for (int c = 0; c < results.length; c++) {
                    ...
                    if (r != null) {
                        generateIndent(System.out, depth);
                        System.out.println("D" + depth + ":  considering move "
                                + r);
                    }
                }
            }
        ...
        if (depth < max_depth) {
            ...
            if (traceLookahead) {
                generateIndent(System.out, depth);
                System.out.println("D" + depth + ": @" + pos
                        + ": evaluating backreferences of " + minLen + "..."
                        + maxLen + " bytes");
            }
            ...
            for (int i = minLen; i <= maxLen; i++) {
                a[i] = selectBestMoveJob(text, backrefs, pos + i, pos + maxLen,
                        max_shortening, depth + 1, max_depth);
            }
            if (mv != null) {
                mv1 = selectBestMoveJob(text, backrefs, pos + 1, pos + 1,
                        max_shortening, depth + 1, max_depth);
            }
            sync();
            int bestGain = -1;
    ...
    public Backref selectBestMoveJob(byte text[], int backrefs[], int pos,
            int bestpos, int max_shortening, int depth, int max_depth) {
        return selectBestMove(text, backrefs, pos, bestpos, max_shortening,
                depth, max_depth);
    }
    ...
    public ByteBuffer compress(byte text[], int max_shortening, int max_depth) {
        ...
        while (pos < text.length
                && (pos < Configuration.MINIMAL_SPAN || backrefs[pos] == -1)) {
            out.append(text[pos++]);
        }
        while (pos + Configuration.MINIMAL_SPAN < text.length) {
            Backref mv = selectBestMove(text, backrefs, pos, pos,
                    max_shortening, 0, max_depth);
            if (mv.backpos < 0) {
    ...
    static void usage() {
        System.err
                .println("Usage: [-quiet] [-verify] [-short <n>] [-depth <n>] <text-file> <compressed-file>");
        System.err
                .println("   or: [-quiet] [-verify] [-short <n>] [-depth <n>] -string <text-string> <compressed-file>");
    }
    ...
    public static void main(String args[]) throws java.io.IOException {
        ...
        if (!quiet) {
            ...
            System.out.println("In: " + text.length + " bytes, out: "
                    + buf.getLength() + " bytes.");
        }
        if (doVerification) {
            ...
            if (nt.length != text.length) {
                System.out
                        .println("Error: decompressed text has different length from original. Original is "
                                + text.length
                                + " bytes, decompression is "
                                + nt.length + " bytes");
                System.exit(1);
            }
            for (int i = 0; i < nt.length; i++) {
                if (nt[i] != text[i]) {
                    System.out
                            .println("Error: decompressed text differs from original at position "
                                    + i);
                    System.exit(1);
                }
            }
